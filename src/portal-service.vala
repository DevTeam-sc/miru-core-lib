	...
	public sealed class PortalService : Object {
			...
			if (controller_connection != null) {
				...
				try {
					...
					entry.take_node_registration (
						node_connection.register_object (ObjectPath.for_miru_message_sink (id), sink));
				} catch (IOError e) {
					assert_not_reached ();
				}
			}
		...
		private class PortalHostSessionProvider : Object, HostSessionProvider {
			...
			public async AgentSession link_agent_session (HostSession host_session, AgentSessionId id, MiruMessageSink sink,
				...
				try {
					entry.take_node_registration (
						entry.node.connection.register_object (ObjectPath.for_miru_message_sink (id), sink));
				} catch (IOError e) {
					assert_not_reached ();
				}
			...
			construct {
				if (connection != null) {
					...
					try {
						...
						registrations.add (
							connection.register_object (Miru.ObjectPath.AUTHENTICATION_SERVICE, null_auth));
					} catch (IOError e) {
						assert_not_reached ();
					}
				}
