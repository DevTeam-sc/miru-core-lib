...
static gboolean miru_add_process_metadata (GHashTable * parameters, guint pid, HANDLE process, MiruEnumerateProcessesOperation * op);

static gboolean miru_get_process_filename (HANDLE process, WCHAR * name, DWORD name_capacity);
static GVariant * miru_get_process_user (HANDLE process);
static GVariant * miru_get_process_start_time (HANDLE process);

static GHashTable * miru_build_ppid_table (void);
static guint miru_get_frontmost_pid (void);

static GDateTime * miru_parse_filetime (const FILETIME * ft);
static gint64 miru_filetime_to_unix (const FILETIME * ft);
...
miru_system_enumerate_processes (MiruProcessQueryOptions * options, int * result_length)
{
  ...
  op.ppid_by_pid = NULL;
  op.frontmost_pid = (op.scope != MIRU_SCOPE_MINIMAL) ? miru_get_frontmost_pid () : 0;
  ...
  else
  {
    ...
    for (i = 0; i != bytes_returned / sizeof (DWORD); i++)
      miru_collect_process_info (pids[i], &op);
...
miru_collect_process_info (guint pid, MiruEnumerateProcessesOperation * op)
{
  ...
  if (!miru_get_process_filename (handle, program_path_utf16, G_N_ELEMENTS (program_path_utf16)))
    goto beach;
  ...
  if (op->scope != MIRU_SCOPE_MINIMAL)
  {
    ...
    still_alive = miru_add_process_metadata (info.parameters, pid, handle, op);
  ...
  if (op->scope == MIRU_SCOPE_FULL)
  {
    ...
    small_icon = _miru_icon_from_process_or_file (pid, program_path_utf16, MIRU_ICON_SMALL);
    if (small_icon != NULL)
    {
    ...
    large_icon = _miru_icon_from_process_or_file (pid, program_path_utf16, MIRU_ICON_LARGE);
    if (large_icon != NULL)
    {
...
static gboolean
miru_add_process_metadata (GHashTable * parameters, guint pid, HANDLE process, MiruEnumerateProcessesOperation * op)
{
  GVariant * user;
  guint ppid;
  GVariant * started;

  user = miru_get_process_user (process);
  if (user == NULL)
    return FALSE;
  g_hash_table_insert (parameters, g_strdup ("user"), g_variant_ref_sink (user));

  if (op->ppid_by_pid == NULL)
    op->ppid_by_pid = miru_build_ppid_table ();
  ...
  started = miru_get_process_start_time (process);
  if (started == NULL)
    return FALSE;
...
static gboolean
miru_get_process_filename (HANDLE process, WCHAR * name, DWORD name_capacity)
{
  gsize name_length;
...
static GVariant *
miru_get_process_user (HANDLE process)
{
...
static GVariant *
miru_get_process_start_time (HANDLE process)
{
  GVariant * result;
  ...
  creation_dt = miru_parse_filetime (&creation_time);
...
static GDateTime *
miru_parse_filetime (const FILETIME * ft)
{
  GDateTime * result;
  gint64 unix_time, unix_sec, unix_usec;
  GDateTime * dt;

  unix_time = miru_filetime_to_unix (ft);
...
static gint64
miru_filetime_to_unix (const FILETIME * ft)
{
  ULARGE_INTEGER u;
