#include "miru-helper-process-glue.h"
...
void *
miru_helper_factory_spawn (const gchar * path, const gchar * parameters, MiruPrivilegeLevel level, GError ** error)
{
  ...
  if (level == MIRU_PRIVILEGE_LEVEL_ELEVATED)
    ei.lpVerb = L"runas";
  else
    ei.lpVerb = L"open";
...
gboolean
miru_helper_instance_is_process_still_running (void * handle)
{
  DWORD exit_code;
...
void
miru_helper_instance_close_process_handle (void * handle)
{
