namespace Miru {
	...
	public sealed class WindowsHostSession : LocalHostSession {
		...
		construct {
			...
			agent = new AgentDescriptor (PathTemplate ("<arch>\\miru-agent.dll"),
				new Bytes.static (Miru.Data.Agent.get_miru_agent_arm64_dll_blob ().data),
				new Bytes.static (Miru.Data.Agent.get_miru_agent_x86_64_dll_blob ().data),
				new Bytes.static (Miru.Data.Agent.get_miru_agent_x86_dll_blob ().data),
				new AgentResource[] {
					new AgentResource ("arm64\\dbghelp.dll",
						new Bytes.static (Miru.Data.Agent.get_dbghelp_arm64_dll_blob ().data), tempdir),
					new AgentResource ("arm64\\symsrv.dll",
						new Bytes.static (Miru.Data.Agent.get_symsrv_arm64_dll_blob ().data), tempdir),
					new AgentResource ("x86_64\\dbghelp.dll",
						new Bytes.static (Miru.Data.Agent.get_dbghelp_x86_64_dll_blob ().data), tempdir),
					new AgentResource ("x86_64\\symsrv.dll",
						new Bytes.static (Miru.Data.Agent.get_symsrv_x86_64_dll_blob ().data), tempdir),
					new AgentResource ("x86\\dbghelp.dll",
						new Bytes.static (Miru.Data.Agent.get_dbghelp_x86_dll_blob ().data), tempdir),
					new AgentResource ("x86\\symsrv.dll",
						new Bytes.static (Miru.Data.Agent.get_symsrv_x86_dll_blob ().data), tempdir),
				},
				tempdir
			);
		}
		...
		protected override async Future<IOStream> perform_attach_to (uint pid, HashTable<string, Variant> options,
			...
			var winjector = injector as Winjector;
			var id = yield winjector.inject_library_resource (pid, agent, "miru_agent_main",
