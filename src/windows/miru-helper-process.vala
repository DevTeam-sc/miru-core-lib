namespace Miru {
	public sealed class WindowsHelperProcess : Object, WindowsHelper {
		...
		public async void inject_library_file (uint pid, PathTemplate path_template, string entrypoint, string data,
			...
			try {
				if (cpu_type_from_pid (pid) == Mumu.NATIVE_CPU) {
	...
	private sealed class HelperFactory {
		...
		public async HelperInstance obtain (Cancellable? cancellable) throws Error, IOError {
			...
			if (obtain_request == null) {
				...
				new Thread<bool> ("miru-helper-factory", obtain_worker);
			}
	...
	private sealed class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			var helper_arm64 = add_helper ("arm64", Miru.Data.Helper.get_miru_helper_arm64_exe_blob (), tempdir);
			var helper_x86_64 = add_helper ("x86_64", Miru.Data.Helper.get_miru_helper_x86_64_exe_blob (), tempdir);
			var helper_x86 = add_helper ("x86", Miru.Data.Helper.get_miru_helper_x86_exe_blob (), tempdir);

			switch (Mumu.Windows.query_native_cpu_type ()) {
				case ARM64:
		...
		private TemporaryFile add_helper (string name, Miru.Data.Helper.Blob blob, TemporaryDirectory tempdir) throws Error {
			var file = new TemporaryFile.from_stream (@"miru-helper-$name.exe",
