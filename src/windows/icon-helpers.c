...
GVariant *
_miru_icon_from_process_or_file (DWORD pid, WCHAR * filename, MiruIconSize size)
{
  GVariant * icon;

  icon = _miru_icon_from_process (pid, size);
  if (icon == NULL)
    icon = _miru_icon_from_file (filename, size);

  return icon;
}

GVariant *
_miru_icon_from_process (DWORD pid, MiruIconSize size)
{
  ...
  if (icon != NULL)
    result = _miru_icon_from_native_icon_handle (icon, size);

  return result;
}

GVariant *
_miru_icon_from_file (WCHAR * filename, MiruIconSize size)
{
  GVariant * result = NULL;
  SHFILEINFOW shfi = { 0, };
  UINT flags;
  ...
  if (shfi.hIcon != NULL)
  {
    result = _miru_icon_from_native_icon_handle (shfi.hIcon, size);
...
GVariant *
_miru_icon_from_resource_url (WCHAR * resource_url, MiruIconSize size)
{
  ...
  *p = L'\0';
  ...
  ret = ExtractIconExW (resource_file, resource_id, (size == MIRU_ICON_LARGE) ? &icon : NULL, (size == MIRU_ICON_SMALL) ? &icon : NULL, 1);
...
GVariant *
_miru_icon_from_native_icon_handle (HICON icon, MiruIconSize size)
{
  GVariant * result;
  HDC dc;
