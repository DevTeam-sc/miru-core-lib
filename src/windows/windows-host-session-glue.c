...
static void miru_child_process_on_death (GPid pid, gint status, gpointer user_data);

static WCHAR * miru_argv_to_command_line (gchar ** argv, gint argv_length);
static WCHAR * miru_envp_to_environment_block (gchar ** envp, gint envp_length);

static void miru_append_n_backslashes (GString * str, guint n);

static void miru_make_pipe (HANDLE * read, HANDLE * write);
static void miru_ensure_not_inherited (HANDLE handle);

GVariant *
_miru_windows_host_session_provider_try_extract_icon (void)
{
  ...
  while (result == NULL && IEnumIDList_Next (children, 1, &child, NULL) == S_OK)
  {
    ...
    result = _miru_icon_from_native_icon_handle (file_info.hIcon, MIRU_ICON_SMALL);
...
MiruChildProcess *
_miru_windows_host_session_spawn (MiruWindowsHostSession * self, const gchar * path, MiruHostSpawnOptions * options, GError ** error)
{
  MiruChildProcess * process = NULL;
  ...
  if (options->has_argv)
    command_line = miru_argv_to_command_line (options->argv, options->argv_length1);
  else
    command_line = NULL;
  ...
  if (options->has_envp || options->has_env)
  {
    ...
    envp = miru_host_spawn_options_compute_envp (options, &envp_length);
    environment = miru_envp_to_environment_block (envp, envp_length);
    g_strfreev (envp);
  }
  else
  {
  ...
  switch (options->stdio)
  {
    ...
    case MIRU_STDIO_PIPE:
      miru_make_pipe (&stdin_read, &stdin_write);
      miru_make_pipe (&stdout_read, &stdout_write);
      miru_make_pipe (&stderr_read, &stderr_write);

      miru_ensure_not_inherited (stdin_write);
      miru_ensure_not_inherited (stdout_read);
      miru_ensure_not_inherited (stderr_read);
  ...
  process = miru_child_process_new (
      G_OBJECT (self),
...
create_process_failed:
  {
    ...
    if (last_error == ERROR_BAD_EXE_FORMAT)
    {
      g_set_error (error,
          MIRU_ERROR,
          MIRU_ERROR_EXECUTABLE_NOT_SUPPORTED,
...
gboolean
_miru_windows_host_session_process_is_alive (guint pid)
{
  HANDLE process;
  DWORD res;
...
void
_miru_child_process_do_close (MiruChildProcess * self)
{
  GSource * watch;

  watch = miru_child_process_get_watch (self);
  if (watch != NULL)
    g_source_destroy (watch);

  CloseHandle (miru_child_process_get_handle (self));
  CloseHandle (miru_child_process_get_main_thread (self));
}

void
_miru_child_process_do_resume (MiruChildProcess * self)
{
  ResumeThread (miru_child_process_get_main_thread (self));
}

static void
miru_child_process_on_death (GPid pid, gint status, gpointer user_data)
{
  MiruChildProcess * self = user_data;

  (void) pid;

  _miru_windows_host_session_on_child_dead (
      MIRU_WINDOWS_HOST_SESSION (miru_child_process_get_parent (self)),
      self,
      status);
}

static WCHAR *
miru_argv_to_command_line (gchar ** argv, gint argv_length)
{
  GString * line;
  WCHAR * line_utf16;
  gint i;
  ...
  for (i = 0; i != argv_length; i++)
  {
    ...
    else
    {
      ...
      for (c = arg; *c != '\0'; c = g_utf8_next_char (c))
      {
        ...
        if (*c == '\0')
        {
          miru_append_n_backslashes (line, num_backslashes * 2);
          break;
        }
        else if (*c == '"')
        {
          miru_append_n_backslashes (line, (num_backslashes * 2) + 1);
          g_string_append_c (line, *c);
        }
        else
        {
          miru_append_n_backslashes (line, num_backslashes);
...
static WCHAR *
miru_envp_to_environment_block (gchar ** envp, gint envp_length)
{
  GString * block;
...
static void
miru_append_n_backslashes (GString * str, guint n)
{
  guint i;
...
static void
miru_make_pipe (HANDLE * read, HANDLE * write)
{
...
static void
miru_ensure_not_inherited (HANDLE handle)
{
