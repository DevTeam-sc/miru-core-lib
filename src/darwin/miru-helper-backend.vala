namespace Miru {
	public class DarwinHelperBackend : Object, DarwinHelper {
		...
		private bool is_booting (uint task) throws Error {
			Mumu.Darwin.AllImageInfos infos;
			try {
				Mumu.Darwin.query_all_image_infos (task, out infos);
			} catch (Mumu.Error e) {
		...
		public void _on_instance_destroyed (void * instance) {
			lock (instances) {
				...
				if (on_last_instance_destroyed != null && instances.is_empty)
					schedule_on_miru_thread ((owned) on_last_instance_destroyed);
			}
		}
		...
		private async void flush_dispatch_queue () {
			_schedule_on_dispatch_queue (() => {
				schedule_on_miru_thread (flush_dispatch_queue.callback);
			});
			yield;
		}
	...
	public sealed class DTraceAgent : Object {
		...
		public void enable_spawn_gating () throws Error {
			...
			string? predicate = Environment.get_variable ("MIRU_DTRACE_PREDICATE");
		...
		private async void process_incoming_messages () {
			try {
				while (true) {
					...
					Mumu.Address caller = uint64.parse (m.fetch (4), 16);
					...
					try {
						...
						Mumu.Darwin.MappingDetails mapping;
						if (Mumu.Darwin.query_mapped_address (task, caller, out mapping)) {
