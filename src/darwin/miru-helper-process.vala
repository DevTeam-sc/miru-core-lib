namespace Miru {
	public sealed class DarwinHelperProcess : Object, DarwinHelper {
		...
		private static void throw_helper_error (GLib.Error e) throws Error, IOError {
			...
			if (e is IOError.CLOSED) {
				throw new Error.PERMISSION_DENIED ("Oops, miru-helper appears to have crashed. It may have been killed " +
					...
					"tss_should_crash=0\"`. For more information, see: https://github.com/miru/miru-core/issues/524");
			}
#endif
	...
	private sealed class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob = Miru.Data.Helper.get_miru_helper_blob ();
			helper = new TemporaryHelperFile (
				new TemporaryFile.from_stream ("miru-helper",
					new MemoryInputStream.from_data (blob.data, null),
					tempdir));
			FileUtils.chmod (helper.path, 0700);
#else
			helper = new InstalledHelperFile.for_path (Config.MIRU_HELPER_PATH);
#endif
		}
