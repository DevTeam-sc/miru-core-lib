namespace Miru {
	public sealed class SimmyHostSessionBackend : Object, HostSessionBackend {
		...
		public async void start (Cancellable? cancellable) throws IOError {
			simmy_context = _start (on_device_added, () => {
				schedule_on_miru_thread (start.callback);
			});
			yield;
		}
		...
		public async void stop (Cancellable? cancellable) throws IOError {
			_stop (simmy_context, () => {
				schedule_on_miru_thread (stop.callback);
			});
			yield;
			simmy_context = null;
	...
	private sealed class SpringboardAgent : InternalAgent {
		...
		protected override async string? load_source (Cancellable? cancellable) throws Error, IOError {
			return (string) Miru.Data.Simmy.get_springboard_js_blob ().data;
		}
	}
	...
	[CCode (gir_namespace = "MiruSimmy", gir_version = "1.0")]
	namespace Simmy {
