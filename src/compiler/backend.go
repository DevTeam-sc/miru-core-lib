...
typedef enum {
  MIRU_OUTPUT_UNESCAPED,
  MIRU_OUTPUT_HEX_BYTES,
  MIRU_OUTPUT_C_STRING,
} MiruOutputFormat;
...
typedef void (* MiruBuildCompleteFunc) (char * bundle, char * error_message, void * user_data);
typedef void (* MiruWatchReadyFunc) (uintptr_t session_handle, char * error_message, void * user_data);
typedef void (* MiruStartingFunc) (void * user_data);
typedef void (* MiruFinishedFunc) (void * user_data);
typedef void (* MiruOutputFunc) (char * bundle, void * user_data);
typedef void (* MiruDiagnosticFunc) (char * category, int code, char * path, int line, int character, char * text,
    void * user_data);
typedef void (* MiruDestroyFunc) (void * user_data);

static inline void
invoke_build_complete_func (MiruBuildCompleteFunc fn,
                            char * bundle,
                            char * error_message,
                            void * user_data)
{
...
static inline void
invoke_watch_ready_func (MiruWatchReadyFunc fn,
...
static inline void
invoke_starting_func (MiruStartingFunc fn,
                      void * user_data)
{
  fn (user_data);
}
...
static inline void
invoke_finished_func (MiruFinishedFunc fn,
                      void * user_data)
{
  fn (user_data);
}
...
static inline void
invoke_output_func (MiruOutputFunc fn,
                    char * bundle,
                    void * user_data)
{
  fn (bundle, user_data);
...
static inline void
invoke_diagnostic_func (MiruDiagnosticFunc fn,
                        char * category,
                        int code,
                        char * path,
                        int line,
...
static inline void
invoke_destroy_func (MiruDestroyFunc fn,
                     void * user_data)
{
  fn (user_data);
}
...
type OutputFormat C.MiruOutputFormat

const (
	OutputFormatUnescaped OutputFormat = OutputFormat(C.MIRU_OUTPUT_UNESCAPED)
	OutputFormatHexBytes  OutputFormat = OutputFormat(C.MIRU_OUTPUT_HEX_BYTES)
	OutputFormatCString   OutputFormat = OutputFormat(C.MIRU_OUTPUT_C_STRING)
)

type BundleFormat C.MiruBundleFormat

const (
	BundleFormatESM  BundleFormat = BundleFormat(C.MIRU_BUNDLE_ESM)
	BundleFormatIIFE BundleFormat = BundleFormat(C.MIRU_BUNDLE_IIFE)
)
...
//export _miru_compiler_backend_build
func _miru_compiler_backend_build(cProjectRoot, cEntrypoint *C.char, outputFormat C.MiruOutputFormat, bundleFormat C.MiruBundleFormat,
	disableTypeCheck, sourceMap, compress uintptr, cPlatform *C.char, cExternals **C.char, numExternals C.int,
	onCompleteFn C.MiruBuildCompleteFunc, onCompleteData unsafe.Pointer, onCompleteDataDestroy C.MiruDestroyFunc,
	onDiagnosticFn C.MiruDiagnosticFunc, onDiagnosticData unsafe.Pointer) {
	options := BuildOptions{
		...
		Compress:         compress != 0,
		Platform:         platformFromMiru(C.GoString(cPlatform)),
...
//export _miru_compiler_backend_watch
func _miru_compiler_backend_watch(cProjectRoot, cEntrypoint *C.char, outputFormat C.MiruOutputFormat, bundleFormat C.MiruBundleFormat,
	disableTypeCheck, sourceMap, compress uintptr, cPlatform *C.char, cExternals **C.char, numExternals C.int,
	onReadyFn C.MiruWatchReadyFunc, onReadyData unsafe.Pointer, onReadyDataDestroy C.MiruDestroyFunc,
	onStartingFn C.MiruStartingFunc, onStartingData unsafe.Pointer,
	onFinishedFn C.MiruFinishedFunc, onFinishedData unsafe.Pointer,
	onOutputFn C.MiruOutputFunc, onOutputData unsafe.Pointer,
	onDiagnosticFn C.MiruDiagnosticFunc, onDiagnosticData unsafe.Pointer) {
	options := BuildOptions{
		ProjectRoot:      C.GoString(cProjectRoot),
		Entrypoint:       C.GoString(cEntrypoint),
...
//export _miru_compiler_backend_watch_session_dispose
func _miru_compiler_backend_watch_session_dispose(h uintptr) {
	handle := cgo.Handle(h)
...
func platformFromMiru(s string) esbuild.Platform {
	switch s {
	case "gum":
...
func makeCBuildDiagnosticCallback(onDiagnostic *CDelegate[C.MiruDiagnosticFunc]) BuildDiagnosticCallback {
...
func makeContext(options BuildOptions, callbacks BuildEventCallbacks) (ctx esbuild.BuildContext, err error) {
	...
	plugins = append(plugins, makeMiruShimsPlugin())
	...
	buildOpts := esbuild.BuildOptions{
		...
		Format:            format,
		Inject:            []string{"miru-builtins:///node-globals.js"},
...
func emitDiagnostic(category string, message esbuild.Message, onDiagnostic BuildDiagnosticCallback) {
	...
	if message.PluginName == "miru-custom-ts" && len(message.Notes) == 2 {
...
type CDelegate[F any] struct {
	...
	null        F
	Data        unsafe.Pointer
	dataDestroy C.MiruDestroyFunc
}

func NewCDelegate[F any](function F, data unsafe.Pointer, dataDestroy C.MiruDestroyFunc) *CDelegate[F] {
