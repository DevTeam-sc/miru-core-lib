namespace Miru {
	public sealed class Compiler : Object {
		...
		construct {
			main_context = Miru.get_main_context ();
		}
		...
		public async string build (string entrypoint, BuildOptions? options = null, Cancellable? cancellable = null)
			...
			try {
				...
				CompilerBackend.BuildCompleteFunc on_complete = (b, e) => {
					bundle = b;
					error_message = e;
					schedule_on_miru_thread (build.callback);
				};
		...
		public async void watch (string entrypoint, WatchOptions? options = null, Cancellable? cancellable = null)
			...
			CompilerBackend.WatchReadyFunc on_ready = (h, e) => {
				session_handle = h;
				error_message = e;
				schedule_on_miru_thread (watch.callback);
			};
		...
		private void init () {
...
#else
			unowned uint8[] backend_so = Miru.Data.Compiler.get_miru_compiler_backend_so_blob ().data;

			Module? backend = null;

			if (MemoryFileDescriptor.is_supported ()) {
				var fd = MemoryFileDescriptor.from_bytes ("miru-compiler-backend.so", new Bytes.static (backend_so));
				try {
			...
			if (backend == null) {
				try {
					string name_used;
					{
						var fd = new FileDescriptor (FileUtils.open_tmp ("miru-compiler-backend-XXXXXX.so", out name_used));
			...
			build = resolve_symbol (backend, "_miru_compiler_backend_build");
			watch = resolve_symbol (backend, "_miru_compiler_backend_watch");
			WatchSession.dispose = resolve_symbol (backend, "_miru_compiler_backend_watch_session_dispose");
#endif
#endif
		}
