namespace Miru {
	public sealed class LinuxHelperProcess : Object, LinuxHelper {
		...
		public async void resume (uint pid, Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				if (!(e is Error.INVALID_ARGUMENT))
					throw e;
				try {
					if (cpu_type == Mumu.CpuType.AMD64 || cpu_type == Mumu.CpuType.ARM64)
		...
		private async LinuxHelper obtain_for_cpu_type (Mumu.CpuType cpu_type, Cancellable? cancellable) throws Error, IOError {
			switch (cpu_type) {
	...
	private sealed class HelperFactory {
		...
		public async LinuxHelper obtain (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				string socket_path = "/miru-" + Uuid.string_random ();
	...
	private sealed class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob32 = Miru.Data.Helper.get_miru_helper_32_blob ();
			if (blob32.data.length > 0)
				helper32 = make_temporary_helper ("miru-helper-32", blob32.data);

			var blob64 = Miru.Data.Helper.get_miru_helper_64_blob ();
			if (blob64.data.length > 0)
				helper64 = make_temporary_helper ("miru-helper-64", blob64.data);
#else
			var tpl = PathTemplate (Config.MIRU_HELPER_PATH);
