namespace Miru {
	...
	public unowned string arch_name_from_pid (uint pid) throws Error {
		Mumu.CpuType cpu_type = cpu_type_from_pid (pid);

		switch (cpu_type) {
			case Mumu.CpuType.IA32:
			case Mumu.CpuType.ARM:
			case Mumu.CpuType.MIPS:
				return "32";

			case Mumu.CpuType.AMD64:
			case Mumu.CpuType.ARM64:
				return "64";
	...
	public Mumu.CpuType cpu_type_from_file (string path) throws Error {
		try {
			return Mumu.Linux.cpu_type_from_file (path);
		} catch (Mumu.Error e) {
			if (e is Mumu.Error.NOT_FOUND)
				throw new Error.EXECUTABLE_NOT_FOUND ("Unable to find executable at '%s'", path);
			else if (e is Mumu.Error.NOT_SUPPORTED)
				throw new Error.EXECUTABLE_NOT_SUPPORTED ("Unable to parse executable at '%s'", path);
			else if (e is Mumu.Error.PERMISSION_DENIED)
	...
	public Mumu.CpuType cpu_type_from_pid (uint pid) throws Error {
		try {
			return Mumu.Linux.cpu_type_from_pid ((Posix.pid_t) pid);
		} catch (Mumu.Error e) {
			if (e is Mumu.Error.NOT_FOUND)
	...
	private sealed class InjectSession : SeizeSession {
		...
		static construct {
			var libc = Mumu.Process.get_libc_module ();
			...
			try {
				var program = new Mumu.ElfModule.from_file ("/proc/self/exe");
		...
		public async RemoteAgent inject (InjectSpec spec, Cancellable? cancellable) throws Error, IOError {
			...
			try {
				unowned uint8[] loader_code = Miru.Data.HelperBackend.get_loader_bin_blob ().data;
		...
		private LoaderLayout compute_loader_layout (InjectSpec spec, string fallback_address) {
			...
			unowned uint8[] code = Miru.Data.HelperBackend.get_loader_bin_blob ().data;
		...
		private async RemoteAgent launch_loader (LoaderLaunch launch, InjectSpec spec, BootstrapResult bres,
			...
			if (loader_result.status != COMPLETED) {
				...
				if (pc >= loader_base && pc < loader_base + Miru.Data.HelperBackend.get_loader_bin_blob ().data.length) {
		...
		private async BootstrapResult bootstrap (size_t loader_size, Cancellable? cancellable) throws Error, IOError {
			...
			unowned uint8[] bootstrapper_code = Miru.Data.HelperBackend.get_bootstrapper_bin_blob ().data;
			...
			try {
				...
				do {
					...
					bootstrap_ctx.allocation_size = allocation_size;
					bootstrap_ctx.page_size = Mumu.query_page_size ();
		...
		private static string make_fallback_address () {
			return "/miru-" + Uuid.string_random ();
		}
	...
	private sealed class RemoteAgent : Object {
		...
		public UnixConnection miru_ctrl {
			get;
			construct;
		}
		...
		private RemoteAgent (uint pid, InjectSpec spec, BootstrapResult bres, UnixConnection miru_ctrl,
		...
		internal static async RemoteAgent start (LoaderLaunch launch, InjectSpec spec, uint pid, BootstrapResult bres,
				UnixConnection miru_ctrl, UnixConnection? agent_ctrl, Cancellable? cancellable) throws Error, IOError {
			var agent = new RemoteAgent (pid, spec, bres, miru_ctrl, agent_ctrl);

			try {
				var io_priority = Priority.DEFAULT;

				if (launch == FROM_SCRATCH)
					miru_ctrl.send_fd (spec.library_so.get_fd (), cancellable);

				if (agent.agent_ctrlfd_for_peer != null) {
					miru_ctrl.send_fd (agent.agent_ctrlfd_for_peer.handle, cancellable);
					agent.agent_ctrlfd_for_peer = null;
				} else {
					yield miru_ctrl.get_output_stream ().write_async ({ 0 }, io_priority, cancellable);
				}
			} catch (GLib.Error e) {
				if (e is IOError.CANCELLED)
					throw (IOError) e;
		...
		public void ack () {
			...
			miru_ctrl.get_output_stream ().write_all_async.begin ((uint8[]) &raw_type, Priority.DEFAULT, null);
		}
		...
		private async void monitor () {
			...
			try {
				...
				InputStream input = miru_ctrl.get_input_stream ();
				var io_priority = Priority.DEFAULT;
				size_t n;
		...
		public RemoteAgent clone (uint clone_id, AgentFeatures features) {
			var agent = new RemoteAgent (0, inject_spec.clone (clone_id, features), bootstrap_result.clone (), miru_ctrl);
			agent._state = _state;
			return agent;
		}
	...
	private sealed class ProcessCodeSwapScope {
		...
		public async ProcessCodeSwapScope (SeizeSession session, uint8[] code, Cancellable? cancellable) throws Error, IOError {
			...
			Mumu.Linux.enumerate_ranges ((Posix.pid_t) session.pid, READ | EXECUTE, d => {
				unowned Mumu.FileMapping? file = d.file;
		...
		switch (err) {
			...
			case Posix.EPERM:
				throw new Error.PERMISSION_DENIED (
					...
					" try `sudo sysctl kernel.yama.ptrace_scope=0`, or run Miru as root",
					pid);
			default:
	...
	private size_t round_size_to_page_size (size_t size) {
		size_t page_size = Mumu.query_page_size ();
