namespace Miru {
	...
	public sealed class LinuxHostSession : LocalHostSession {
		...
		construct {
			...
			var blob32 = Miru.Data.Agent.get_miru_agent_32_so_blob ();
			var blob64 = Miru.Data.Agent.get_miru_agent_64_so_blob ();
			var emulated_arm = Miru.Data.Agent.get_miru_agent_arm_so_blob ();
			var emulated_arm64 = Miru.Data.Agent.get_miru_agent_arm64_so_blob ();
			agent = new AgentDescriptor (PathTemplate ("miru-agent-<arch>.so"),
				new Bytes.static (blob32.data),
				new Bytes.static (blob64.data),
				new AgentResource[] {
					new AgentResource ("miru-agent-arm.so", new Bytes.static (emulated_arm.data), tempdir),
					new AgentResource ("miru-agent-arm64.so", new Bytes.static (emulated_arm64.data), tempdir),
				},
				AgentMode.INSTANCED,
				tempdir);
#endif
...
#else
			tpl = PathTemplate (Config.MIRU_AGENT_PATH);
#endif
			if (path == null)
		...
		protected override async Future<IOStream> perform_attach_to (uint pid, HashTable<string, Variant> options,
			...
			uint id;
			string entrypoint = "miru_agent_main";
...
#else
			id = yield linjector.inject_library_file_with_template (pid, PathTemplate (Config.MIRU_AGENT_PATH), entrypoint,
		...
		internal async AndroidHelperClient get_android_helper_client (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				helper_path = "/data/local/tmp/miru-helper-" + instance_id + ".dex";
				FileUtils.set_data (helper_path, Miru.Data.Android.get_helper_dex_blob ().data);
				Posix.chmod (helper_path, 0644);

				var helper_address = new UnixSocketAddress.with_type ("/miruk-helper-" + instance_id, -1, ABSTRACT);
	...
	private sealed class RoboLauncher : Object {
		...
		private async void ensure_loaded (Cancellable? cancellable) throws Error, IOError {
			...
			if (server_name == null) {
				string name = "/miru-zymbiote-" + Uuid.string_random ().replace ("-", "");
		...
		private static ZymbiotePrepResult do_prepare_zymbiote_injection (uint pid, string server_name) throws Error, IOError {
			...
			Gee.List<Mumu.MemoryRange?> heap_candidates = new Gee.ArrayList<Mumu.MemoryRange?> ();
			...
			while (iter.next ()) {
				...
				if (path.has_suffix ("/libstagefright.so") && "x" in flags) {
					if (payload_base == 0) {
						payload_base = iter.end_address - Mumu.query_page_size ();
						payload_path = path;
					...
					heap_candidates.add (Mumu.MemoryRange () {
						base_address = start,
			...
			Mumu.ElfModule libc;
			try {
				libc = new Mumu.ElfModule.from_file (libc_path);
			} catch (Mumu.Error e) {
				throw new Error.NOT_SUPPORTED ("Unable to parse libc.so: %s", e.message);
			}

			Mumu.ElfModule runtime;
			try {
				runtime = new Mumu.ElfModule.from_file (runtime_path);
			} catch (Mumu.Error e) {
...
#if ARM || ARM64
				? Miru.Data.Android.get_zymbiote_arm64_bin_blob ()
				: Miru.Data.Android.get_zymbiote_arm_bin_blob ();
#else
				? Miru.Data.Android.get_zymbiote_x86_64_bin_blob ()
				: Miru.Data.Android.get_zymbiote_x86_bin_blob ();
#endif

			unowned uint8[] payload_template = blob.data;
			void * p = memmem (payload_template, "/miru-socket-00000000000000000000000000000000".data);
			assert (p != null);
