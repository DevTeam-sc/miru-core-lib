[CCode (gir_namespace = "MiruGDB", gir_version = "1.0")]
namespace Miru.GDB {
	public class Client : Object, AsyncInitable {
		...
		private class FeatureDocument {
			...
			private class Parser {
					...
					foreach (unowned string attribute_name in attribute_names) {
						...
						if (attribute_name == "name") {
							if (val.has_prefix ("com.apple.debugserver."))
								doc.arch = parse_mldb_arch (val[22:]);
							return;
						}
				...
				private static TargetArch parse_mdb_arch (string name) {
					switch (name) {
				...
				private static TargetArch parse_mldb_arch (string name) {
					if (name == "i386")
						return IA32;
