namespace Miru {
	...
	public interface HostSessionProvider : Object {
		...
		public abstract async AgentSession link_agent_session (HostSession host_session, AgentSessionId id, MiruMessageSink sink,
	...
	public interface HostSessionConnection : Object {
		...
		public abstract async AgentSession link_agent_session (AgentSessionId id, MiruMessageSink sink,
			Cancellable? cancellable) throws Error, IOError;
	...
	public abstract class LocalHostSession : Object, HostSession, HostSessionConnection, AgentController {
		...
		private async AgentEntry establish (uint pid, HashTable<string, Variant> options,
			...
			try {
				...
				if (e is IOError.CANCELLED && (cancellable == null || !cancellable.is_cancelled ())) {
					e = new Error.PROCESS_NOT_RESPONDING ("Process with pid %u either refused to load miru-agent, " +
		...
		public async AgentSession link_agent_session (AgentSessionId id, MiruMessageSink sink,
			...
			try {
				entry.sink_registration_id = connection.register_object (ObjectPath.for_miru_message_sink (id), sink);
			} catch (IOError e) {
				assert_not_reached ();
			}
	...
	public abstract class InternalAgent : Object, MiruMessageSink, RpcPeer {
		...
		protected async void ensure_loaded (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				try {
					...
					session = yield connection.link_agent_session (session_id, (MiruMessageSink) this, cancellable);
