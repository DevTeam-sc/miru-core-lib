...
namespace Miru.Barebone {
	public sealed class ArmMachine : Object, Machine {
		...
		public ArmMachine (GDB.Client mdb) {
			Object (mdb: mdb);
		}
		...
		public async void protect_pages (uint64 virtual_address, size_t size, Mumu.PageProtection prot,
		...
		public async CallFrame load_call_frame (GDB.Thread thread, uint arity, Cancellable? cancellable) throws Error, IOError {
			...
			if (arity > NUM_ARGS_IN_REGS)
				stack = yield mdb.read_buffer (original_sp, (arity - NUM_ARGS_IN_REGS) * 4, cancellable);
