...
namespace Miru.Barebone {
	public sealed class Layout : Object {
		...
		public static async Layout load_from_symbol_source (File symbol_source, uint64 kernel_base, ByteOrder byte_order,
			...
			Mumu.DarwinModule mod;
			try {
				mod = new Mumu.DarwinModule.from_blob (kerncache, ARM64, Mumu.PtrauthSupport.SUPPORTED);
			} catch (Mumu.Error e) {
		...
		private static Gee.List<ModuleInfo> compute_module_list (Mumu.DarwinModule mod, Blob blob, uint64 kernel_base) throws Error {
		...
		private static Gee.List<SymbolInfo> compute_symbol_list (Mumu.DarwinModule mod) {
