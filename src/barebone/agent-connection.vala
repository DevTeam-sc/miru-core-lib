...
namespace Miru.Barebone {
	public sealed class AgentConnection : Object, AsyncInitable {
		...
		private async bool init_async (int io_priority, Cancellable? cancellable) throws Error, IOError {
			...
			var mdb = machine.mdb;
			ByteOrder byte_order = mdb.byte_order;
			uint pointer_size = mdb.pointer_size;
			...
			Mumu.ElfModule elf;
			try {
				elf = new Mumu.ElfModule.from_file (agent_config.path);
			} catch (Mumu.Error e) {
				throw new Error.INVALID_ARGUMENT ("%s", e.message);
			}

			var raw_elf = mdb.make_buffer (new Bytes (elf.get_file_data ()));
			var missing_symbols = new Gee.ArrayList<string> ();
			elf.enumerate_symbols (s => {
				unowned Mumu.ElfSectionDetails? sect = s.section;
			...
			var bp = yield mdb.add_breakpoint (SOFT, kernel_base + thread_block.offset, 4, cancellable);
			GDB.Breakpoint? hit_breakpoint = null;
			do {
				var exception = yield mdb.continue_until_exception (cancellable);
			...
			elf.enumerate_symbols (e => {
				...
				else if (e.name == "mumu_barebone_try_remap_writable_pages")
					remap_writable_pages_address = base_va + e.address;
				else if (e.name == "mumu_try_mprotect")
			...
			if (remap_writable_pages_address == 0)
				throw new Error.INVALID_ARGUMENT ("Invalid agent: no mumu_barebone_try_remap_writable_pages symbol found");
			if (mprotect_address == 0)
				throw new Error.INVALID_ARGUMENT ("Invalid agent: no mumu_try_mprotect symbol found");
			...
			yield mdb.write_byte_array (config_allocation.virtual_address, config_blob, cancellable);
		...
		private async void process_incoming_messages () {
			var byte_order = machine.mdb.byte_order;
		...
		private class RemapWritablePagesHandler : Object, CallbackHandler {
			...
			public async uint64 handle_invocation (uint64[] args, CallFrame frame, Cancellable? cancellable)
				...
				var mdb = machine.mdb;
				var reader = new BufferReader (yield mdb.read_buffer (pages, num_pages * mdb.pointer_size, cancellable));
		...
		private class MemoryProtectHandler : Object, CallbackHandler {
			...
			public async uint64 handle_invocation (uint64[] args, CallFrame frame, Cancellable? cancellable)
				...
				var prot = (Mumu.PageProtection) args[2];
				try {
