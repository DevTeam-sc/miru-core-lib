...
namespace Miru.Barebone {
	public interface Machine : Object {
		public abstract GDB.Client mdb {
			get;
			set;
		}
		...
		public abstract async void enumerate_ranges (Mumu.PageProtection prot, FoundRangeFunc func, Cancellable? cancellable)
			throws Error, IOError;

		public abstract async Allocation allocate_pages (Gee.List<uint64?> physical_addresses, Cancellable? cancellable)
			throws Error, IOError;

		public abstract async void protect_pages (uint64 virtual_address, size_t size, Mumu.PageProtection prot,
			Cancellable? cancellable) throws Error, IOError;

		public abstract async Gee.List<uint64?> scan_ranges (Gee.List<Mumu.MemoryRange?> ranges, MatchPattern pattern,
			uint max_matches, Cancellable? cancellable) throws Error, IOError;

		public Bytes relocate (Mumu.ElfModule elf, Bytes raw_elf, uint64 base_va) throws Error {
			...
			var relocated_buf = mdb.make_buffer (new Bytes (raw_elf[(size_t) file_start:(size_t) file_end].get_data ()));
			...
			Bytes relocated_image = mdb.make_buffer_builder ()
		...
		public abstract void apply_relocation (Mumu.ElfRelocationDetails r, uint64 base_va, Buffer relocated) throws Error;
	...
	public class RangeDetails {
		...
		public uint64 size;
		public Mumu.PageProtection protection;
		...
		public RangeDetails (uint64 base_va, uint64 base_pa, uint64 size, Mumu.PageProtection protection, MappingType type) {
