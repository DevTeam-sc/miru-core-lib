namespace Miru {
	...
	public sealed class BareboneHostSessionProvider : Object, HostSessionProvider {
		...
		public async HostSession create (HostSessionHub hub, HostSessionOptions? options, Cancellable? cancellable)
			...
			unowned string? config_path = Environment.get_variable ("MIRU_BAREBONE_CONFIG");
			if (config_path != null) {
				try {
			...
			var mdb = yield GDB.Client.open (stream, cancellable);

			Barebone.Machine machine;
			switch (mdb.arch) {
				case IA32:
					machine = new Barebone.IA32Machine (mdb);
					break;
				case X64:
					machine = new Barebone.X64Machine (mdb);
					break;
				case ARM:
					machine = new Barebone.ArmMachine (mdb);
					break;
				case ARM64:
					machine = new Barebone.Arm64Machine (mdb);
					break;
				default:
					machine = new Barebone.UnknownMachine (mdb);
					break;
			}
	...
	private abstract class BareboneAgentSession : Object, AgentSession {
		...
		construct {
			...
			transmitter = new AgentMessageTransmitter (this, persist_timeout, miru_context, dbus_context);
