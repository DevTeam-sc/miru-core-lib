namespace Miru {
	private sealed class BareboneScript : Object {
		...
		private static QuickJS.ClassID mdb_thread_class;

		private static QuickJS.ClassID mdb_breakpoint_class;
		private Gee.Map<GDB.Breakpoint, QuickJS.Value?> mdb_breakpoints = new Gee.HashMap<GDB.Breakpoint, QuickJS.Value?> ();
		...
		public static BareboneScript create (AgentScriptId id, string source, Barebone.Services services) throws Error {
			...
			unowned string runtime_js = (string) Miru.Data.Barebone.get_script_runtime_js_blob ().data;
			script.add_program (runtime_js, "/_miru.js");
		...
		construct {
			...
			process_obj.set_property_str (ctx, "arch", ctx.make_string (mdb.arch.to_nick ()));
			process_obj.set_property_str (ctx, "pageSize", ctx.make_uint32 ((uint32) services.allocator.page_size));
			process_obj.set_property_str (ctx, "pointerSize", ctx.make_uint32 (mdb.pointer_size));
			...
			var mdb_obj = ctx.make_object ();
			add_getter (mdb_obj, "state", on_mdb_get_state);
			add_getter (mdb_obj, "exception", on_mdb_get_exception);
			add_cfunc (mdb_obj, "continue", on_mdb_continue, 0);
			add_cfunc (mdb_obj, "stop", on_mdb_stop, 0);
			add_cfunc (mdb_obj, "restart", on_mdb_restart, 0);
			add_cfunc (mdb_obj, "readPointer", on_mdb_read_pointer, 1);
			add_cfunc (mdb_obj, "writePointer", on_mdb_write_pointer, 2);
			add_cfunc (mdb_obj, "readS8", on_mdb_read_s8, 1);
			add_cfunc (mdb_obj, "writeS8", on_mdb_write_s8, 2);
			add_cfunc (mdb_obj, "readU8", on_mdb_read_u8, 1);
			add_cfunc (mdb_obj, "writeU8", on_mdb_write_u8, 2);
			add_cfunc (mdb_obj, "readS16", on_mdb_read_s16, 1);
			add_cfunc (mdb_obj, "writeS16", on_mdb_write_s16, 2);
			add_cfunc (mdb_obj, "readU16", on_mdb_read_u16, 1);
			add_cfunc (mdb_obj, "writeU16", on_mdb_write_u16, 2);
			add_cfunc (mdb_obj, "readS32", on_mdb_read_s32, 1);
			add_cfunc (mdb_obj, "writeS32", on_mdb_write_s32, 2);
			add_cfunc (mdb_obj, "readU32", on_mdb_read_u32, 1);
			add_cfunc (mdb_obj, "writeU32", on_mdb_write_u32, 2);
			add_cfunc (mdb_obj, "readS64", on_mdb_read_s64, 1);
			add_cfunc (mdb_obj, "writeS64", on_mdb_write_s64, 2);
			add_cfunc (mdb_obj, "readU64", on_mdb_read_u64, 1);
			add_cfunc (mdb_obj, "writeU64", on_mdb_write_u64, 2);
			add_cfunc (mdb_obj, "readFloat", on_mdb_read_float, 1);
			add_cfunc (mdb_obj, "writeFloat", on_mdb_write_float, 2);
			add_cfunc (mdb_obj, "readDouble", on_mdb_read_double, 1);
			add_cfunc (mdb_obj, "writeDouble", on_mdb_write_double, 2);
			add_cfunc (mdb_obj, "readByteArray", on_mdb_read_byte_array, 2);
			add_cfunc (mdb_obj, "writeByteArray", on_mdb_write_byte_array, 2);
			add_cfunc (mdb_obj, "readCString", on_mdb_read_c_string, 2);
			add_cfunc (mdb_obj, "readUtf8String", on_mdb_read_utf8_string, 2);
			add_cfunc (mdb_obj, "writeUtf8String", on_mdb_write_utf8_string, 2);
			add_cfunc (mdb_obj, "addBreakpoint", on_mdb_add_breakpoint, 3);
			add_cfunc (mdb_obj, "runRemoteCommand", on_mdb_run_remote_command, 1);
			add_cfunc (mdb_obj, "execute", on_mdb_execute, 1);
			add_cfunc (mdb_obj, "query", on_mdb_query, 1);
			global.set_property_str (ctx, "$mdb", mdb_obj);

			QuickJS.ClassDef th;
			th.class_name = "GDBThread";
			th.finalizer = on_mdb_thread_finalize;
			rt.make_class (QuickJS.make_class_id (ref mdb_thread_class), th);
			var th_proto = ctx.make_object ();
			add_getter (th_proto, "id", on_mdb_thread_get_id);
			add_getter (th_proto, "name", on_mdb_thread_get_name);
			add_cfunc (th_proto, "step", on_mdb_thread_step, 0);
			add_cfunc (th_proto, "stepAndContinue", on_mdb_thread_step_and_continue, 0);
			add_cfunc (th_proto, "readRegisters", on_mdb_thread_read_registers, 0);
			add_cfunc (th_proto, "readRegister", on_mdb_thread_read_register, 1);
			add_cfunc (th_proto, "writeRegister", on_mdb_thread_write_register, 2);
			ctx.set_class_proto (mdb_thread_class, th_proto);

			QuickJS.ClassDef bp;
			bp.class_name = "GDBBreakpoint";
			bp.finalizer = on_mdb_breakpoint_finalize;
			rt.make_class (QuickJS.make_class_id (ref mdb_breakpoint_class), bp);
			var bp_proto = ctx.make_object ();
			add_getter (bp_proto, "kind", on_mdb_breakpoint_get_kind);
			add_getter (bp_proto, "address", on_mdb_breakpoint_get_address);
			add_getter (bp_proto, "size", on_mdb_breakpoint_get_size);
			add_cfunc (bp_proto, "enable", on_mdb_breakpoint_enable, 0);
			add_cfunc (bp_proto, "disable", on_mdb_breakpoint_disable, 0);
			add_cfunc (bp_proto, "remove", on_mdb_breakpoint_remove, 0);
			ctx.set_class_proto (mdb_breakpoint_class, bp_proto);
		}
		...
		private async void do_memory_alloc (size_t size, Promise<Barebone.Allocation> promise) {
			try {
				...
				Bytes zeroes = mdb.make_buffer_builder ()
					.skip (size)
					.build ();
				yield mdb.write_byte_array (allocation.virtual_address, zeroes, io_cancellable);
		...
		private async void do_memory_protect (uint64 address, size_t size, Mumu.PageProtection prot, Promise<bool?> promise) {
			try {
		...
		private class ScanArgs {
			public Gee.List<Mumu.MemoryRange?> ranges = new Gee.ArrayList<Mumu.MemoryRange?> ();
		...
		private async void do_process_enumerate_ranges (Mumu.PageProtection prot, Promise<QuickJS.Value?> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_get_state (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();

			return ctx.make_string (script->mdb.state.to_nick ());
		}

		private static QuickJS.Value on_mdb_get_exception (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();

			GDB.Exception? exception = script->mdb.exception;
			if (exception == null)
				return QuickJS.Null;

			var result = ctx.make_object ();
			result.set_property (ctx, script->signum_key, ctx.make_uint32 (exception.signum));
			result.set_property (ctx, script->breakpoint_key, script->wrap_mdb_breakpoint_nullable (exception.breakpoint));
			result.set_property (ctx, script->thread_key, script->wrap_mdb_thread (exception.thread));
			return result;
		}

		private static QuickJS.Value on_mdb_continue (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();

			var promise = new Promise<GDB.Client> ();
			script->do_mdb_continue.begin (promise);
		...
		private async void do_mdb_continue (Promise<GDB.Client> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_stop (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();

			var promise = new Promise<GDB.Client> ();
			script->do_mdb_stop.begin (promise);
		...
		private async void do_mdb_stop (Promise<GDB.Client> promise) {
			try {
				yield mdb.stop (io_cancellable);
		...
		private static QuickJS.Value on_mdb_restart (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
		...
		private static QuickJS.Value on_mdb_read_pointer (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, script->mdb.pointer_size, script->parse_raw_pointer);
		}

		private static QuickJS.Value on_mdb_write_pointer (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_pointer);
		}

		private static QuickJS.Value on_mdb_read_s8 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 1, script->parse_raw_s8);
		}

		private static QuickJS.Value on_mdb_write_s8 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_s8);
		}

		private static QuickJS.Value on_mdb_read_u8 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 1, script->parse_raw_u8);
		}

		private static QuickJS.Value on_mdb_write_u8 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_u8);
		}

		private static QuickJS.Value on_mdb_read_s16 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 2, script->parse_raw_s16);
		}

		private static QuickJS.Value on_mdb_write_s16 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_s16);
		}

		private static QuickJS.Value on_mdb_read_u16 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 2, script->parse_raw_u16);
		}

		private static QuickJS.Value on_mdb_write_u16 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_u16);
		}

		private static QuickJS.Value on_mdb_read_s32 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 4, script->parse_raw_s32);
		}

		private static QuickJS.Value on_mdb_write_s32 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_s32);
		}

		private static QuickJS.Value on_mdb_read_u32 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 4, script->parse_raw_u32);
		}

		private static QuickJS.Value on_mdb_write_u32 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_u32);
		}

		private static QuickJS.Value on_mdb_read_s64 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 8, script->parse_raw_s64);
		}

		private static QuickJS.Value on_mdb_write_s64 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_s64);
		}

		private static QuickJS.Value on_mdb_read_u64 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 8, script->parse_raw_u64);
		}

		private static QuickJS.Value on_mdb_write_u64 (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_u64);
		}

		private static QuickJS.Value on_mdb_read_float (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 4, script->parse_raw_float);
		}

		private static QuickJS.Value on_mdb_write_float (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_float);
		}

		private static QuickJS.Value on_mdb_read_double (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_read (ctx, this_val, argv, 8, script->parse_raw_double);
		}

		private static QuickJS.Value on_mdb_write_double (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_double);
		}

		private static QuickJS.Value on_mdb_read_byte_array (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			...
			return script->do_mdb_read (ctx, this_val, argv, size, script->parse_raw_byte_array);
		}

		private static QuickJS.Value on_mdb_write_byte_array (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_byte_array);
		}

		private static QuickJS.Value on_mdb_read_c_string (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();

			ByteArray? bytes = script->do_mdb_read_null_terminated_string (ctx, this_val, argv);
			if (bytes == null)
				return QuickJS.Exception;
		...
		private static QuickJS.Value on_mdb_read_utf8_string (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
		...
		private ByteArray? do_mdb_read_null_terminated_string (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			uint64 address;
		...
		private static QuickJS.Value on_mdb_write_utf8_string (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			return script->do_mdb_write (ctx, this_val, argv, script->unparse_raw_utf8_string);
		}
		...
		private QuickJS.Value do_mdb_read (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv, uint size,
			...
			return parse (mdb.make_buffer (bytes));
		}

		private delegate QuickJS.Value GdbReadResultParseFunc (Buffer buffer);

		private QuickJS.Value do_mdb_write (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv,
				GdbWriteUnparseFunc unparse) {
			uint64 address;
			if (!unparse_uint64 (argv[0], out address))
				return QuickJS.Exception;

			BufferBuilder? builder = unparse (argv[1], mdb.make_buffer_builder ());
			if (builder == null)
				return QuickJS.Exception;
		...
		private async void do_read_memory (uint64 address, uint size, Promise<Bytes> promise) {
			try {
				Bytes bytes = yield mdb.read_byte_array (address, size, io_cancellable);
		...
		private async void do_write_memory (uint64 address, Bytes bytes, Promise<GDB.Client> promise) {
			try {
				yield mdb.write_byte_array (address, bytes, io_cancellable);
		...
		private static QuickJS.Value on_mdb_add_breakpoint (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			...
			script->do_mdb_add_breakpoint.begin (kind, address, size, promise);

			GDB.Breakpoint? bp = script->process_events_until_ready (promise);
			if (bp == null)
				return QuickJS.Exception;

			return script->wrap_mdb_breakpoint (bp);
		}

		private async void do_mdb_add_breakpoint (GDB.Breakpoint.Kind kind, uint64 address, uint size,
				Promise<GDB.Breakpoint> promise) {
			try {
				GDB.Breakpoint bp = yield mdb.add_breakpoint (kind, address, size, io_cancellable);
		...
		private static QuickJS.Value on_mdb_run_remote_command (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			...
			script->do_mdb_run_remote_command.begin (command, promise);
		...
		private async void do_mdb_run_remote_command (string command, Promise<string> promise) {
			try {
				string result = yield mdb.run_remote_command (command, io_cancellable);
		...
		private static QuickJS.Value on_mdb_execute (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			...
			var promise = new Promise<GDB.Client> ();
			script->do_mdb_execute.begin (command, promise);
		...
		private async void do_mdb_execute (string command, Promise<GDB.Client> promise) {
			try {
				yield mdb.execute_simple (command, io_cancellable);
		...
		private static QuickJS.Value on_mdb_query (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			...
			script->do_mdb_query.begin (request, promise);
		...
		private async void do_mdb_query (string request, Promise<GDB.Client.Packet> promise) {
			try {
				GDB.Client.Packet packet = yield mdb.query_simple (request, io_cancellable);
		...
		private QuickJS.Value wrap_mdb_thread (GDB.Thread thread) {
			var wrapper = ctx.make_object_class (mdb_thread_class);
			wrapper.set_opaque (thread.ref ());
			return wrapper;
		}

		private static void on_mdb_thread_finalize (QuickJS.Runtime rt, QuickJS.Value val) {
			GDB.Thread * thread = val.get_opaque (mdb_thread_class);
			thread->unref ();
		}

		private static QuickJS.Value on_mdb_thread_get_id (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);
			return ctx.make_string (thread->id);
		}

		private static QuickJS.Value on_mdb_thread_get_name (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);
		...
		private static QuickJS.Value on_mdb_thread_step (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);

			var promise = new Promise<GDB.Thread> ();
			script->do_mdb_thread_step.begin (thread, promise);
		...
		private async void do_mdb_thread_step (GDB.Thread thread, Promise<GDB.Thread> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_thread_step_and_continue (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);
		...
		private static QuickJS.Value on_mdb_thread_read_registers (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);

			var promise = new Promise<Gee.Map<string, Variant>> ();
			script->do_mdb_thread_read_registers.begin (thread, promise);
		...
		private async void do_mdb_thread_read_registers (GDB.Thread thread, Promise<Gee.Map<string, Variant>> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_thread_read_register (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);
			...
			var promise = new Promise<uint64?> ();
			script->do_mdb_thread_read_register.begin (thread, name, promise);
		...
		private async void do_mdb_thread_read_register (GDB.Thread thread, string name, Promise<uint64?> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_thread_write_register (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Thread * thread = this_val.get_opaque (mdb_thread_class);
			...
			var promise = new Promise<GDB.Thread> ();
			script->do_mdb_thread_write_register.begin (thread, name, val, promise);
		...
		private async void do_mdb_thread_write_register (GDB.Thread thread, string name, uint64 val, Promise<GDB.Thread> promise) {
			try {
		...
		private QuickJS.Value wrap_mdb_breakpoint_nullable (GDB.Breakpoint? bp) {
			if (bp == null)
				return QuickJS.Null;
			return wrap_mdb_breakpoint (bp);
		}

		private QuickJS.Value wrap_mdb_breakpoint (GDB.Breakpoint bp) {
			QuickJS.Value? existing_wrapper = mdb_breakpoints[bp];
			if (existing_wrapper != null)
				return ctx.dup_value (existing_wrapper);

			var wrapper = ctx.make_object_class (mdb_breakpoint_class);
			wrapper.set_opaque (bp.ref ());
			mdb_breakpoints[bp] = wrapper;

			return wrapper;
		}

		private static void on_mdb_breakpoint_finalize (QuickJS.Runtime rt, QuickJS.Value val) {
			GDB.Breakpoint * bp = val.get_opaque (mdb_breakpoint_class);
			bp->unref ();
		}

		private static QuickJS.Value on_mdb_breakpoint_get_kind (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			GDB.Breakpoint * bp = this_val.get_opaque (mdb_breakpoint_class);
			return ctx.make_string (bp->kind.to_nick ());
		}

		private static QuickJS.Value on_mdb_breakpoint_get_address (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Breakpoint * bp = this_val.get_opaque (mdb_breakpoint_class);
		...
		private static QuickJS.Value on_mdb_breakpoint_get_size (QuickJS.Context ctx, QuickJS.Value this_val,
				QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Breakpoint * bp = this_val.get_opaque (mdb_breakpoint_class);
		...
		private static QuickJS.Value on_mdb_breakpoint_enable (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Breakpoint * bp = this_val.get_opaque (mdb_breakpoint_class);

			var promise = new Promise<GDB.Breakpoint> ();
			script->do_mdb_breakpoint_enable.begin (bp, promise);
		...
		private async void do_mdb_breakpoint_enable (GDB.Breakpoint bp, Promise<GDB.Breakpoint> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_breakpoint_disable (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Breakpoint * bp = this_val.get_opaque (mdb_breakpoint_class);

			var promise = new Promise<GDB.Breakpoint> ();
			script->do_mdb_breakpoint_disable.begin (bp, promise);
		...
		private async void do_mdb_breakpoint_disable (GDB.Breakpoint bp, Promise<GDB.Breakpoint> promise) {
			try {
		...
		private static QuickJS.Value on_mdb_breakpoint_remove (QuickJS.Context ctx, QuickJS.Value this_val, QuickJS.Value[] argv) {
			BareboneScript * script = ctx.get_opaque ();
			GDB.Breakpoint * bp = this_val.get_opaque (mdb_breakpoint_class);

			var promise = new Promise<GDB.Breakpoint> ();
			script->do_mdb_breakpoint_remove.begin (bp, promise);
		...
		private async void do_mdb_breakpoint_remove (GDB.Breakpoint bp, Promise<GDB.Breakpoint> promise) {
			try {
		...
		private QuickJS.Value parse_page_protection (Mumu.PageProtection prot) {
			char str[4] = {
				((prot & Mumu.PageProtection.READ) != 0) ? 'r' : '-',
				((prot & Mumu.PageProtection.WRITE) != 0) ? 'w' : '-',
				((prot & Mumu.PageProtection.EXECUTE) != 0) ? 'x' : '-',
				'\0'
			};
			return ctx.make_string ((string) str);
		}

		private bool unparse_page_protection (QuickJS.Value val, out Mumu.PageProtection prot) {
			prot = NO_ACCESS;
