...
namespace Miru.Barebone {
	public sealed class Arm64Machine : Object, Machine {
		public override GDB.Client mdb {
			get;
			set;
		}
		...
		public Arm64Machine (GDB.Client mdb) {
			Object (mdb: mdb);
		}

		public async size_t query_page_size (Cancellable? cancellable) throws Error, IOError {
			MMUParameters p = yield MMUParameters.load (mdb, cancellable);
		...
		public async void enumerate_ranges (Mumu.PageProtection prot, FoundRangeFunc func, Cancellable? cancellable)
				throws Error, IOError {
			Gee.List<RangeDetails> ranges = ("corellium" in mdb.features)
		...
		private async Gee.List<RangeDetails> collect_ranges_using_mmu (Cancellable? cancellable) throws Error, IOError {
			...
			yield set_addressing_mode (mdb, PHYSICAL, cancellable);
			try {
				yield collect_ranges_in_table (p.tt1, p.first_level, p.upper_bits, p, result, cancellable);
			} finally {
				set_addressing_mode.begin (mdb, VIRTUAL, null);
			}
		...
		private async void collect_ranges_in_table (uint64 table_address, uint level, uint64 upper_bits, MMUParameters p,
			...
			Buffer entries = yield mdb.read_buffer (table_address, max_entries * Descriptor.SIZE, cancellable);
			...
			for (uint i = 0; i != max_entries; i++) {
				...
				if (desc.kind == BLOCK) {
					...
					Mumu.PageProtection prot = protection_from_flags (desc.flags, p);
		...
		private async Gee.List<RangeDetails> collect_ranges_using_corellium (Cancellable? cancellable) throws Error, IOError {
			...
			string pt = yield mdb.run_remote_command ("pt", cancellable);
			foreach (string line in pt.split ("\n")) {
				...
				Mumu.PageProtection prot = protection_from_corellium_pt_details (details);
		...
		private async Allocation? maybe_insert_descriptor_in_table (Gee.List<uint64?> physical_addresses,
			...
			while (chunk_offset != max_entries && num_available_slots != num_pages) {
				...
				Buffer descriptors = yield mdb.read_buffer (chunk_base_address, chunk_size * Descriptor.SIZE, cancellable);
			...
			Mumu.PageProtection page_prot = Mumu.PageProtection.READ | Mumu.PageProtection.WRITE;
			if (!p.sprr_enabled)
				page_prot |= Mumu.PageProtection.EXECUTE;

			var builder = mdb.make_buffer_builder ();
			...
			Bytes old_descriptors = yield mdb.read_byte_array (first_available_slot, new_descriptors.get_size (), cancellable);
			yield mdb.write_byte_array (first_available_slot, new_descriptors, cancellable);

			size_t size = num_pages * p.granule;
			return new DescriptorAllocation (first_available_va, size, first_available_slot, old_descriptors, mdb);
		}

		public async void protect_pages (uint64 virtual_address, size_t size, Mumu.PageProtection prot, Cancellable? cancellable)
				throws Error, IOError {
			MMUParameters p = yield MMUParameters.load (mdb, cancellable);
		...
		private async void perform_protect_pages (uint64 start_va, uint num_pages, Mumu.PageProtection prot, MMUParameters p,
			...
			bool need_exec = (prot & Mumu.PageProtection.EXECUTE) != 0;
		...
		private async void protect_pages_in_table (uint64 current_va, uint batch_size, Mumu.PageProtection prot,
			...
			Buffer descriptors = yield mdb.read_buffer (first_slot_pa, batch_size * Descriptor.SIZE, cancellable);
			...
			if (any_changed)
				yield mdb.write_byte_array (first_slot_pa, descriptors.bytes, cancellable);
		}
		...
		private async uint64 walk_to_level3 (uint64 va, uint64 start_table_pa, uint start_level, MMUParameters p,
			...
			while (level != 3) {
				...
				Buffer d_buf = yield mdb.read_buffer (slot_pa, Descriptor.SIZE, cancellable);
		...
		private async void clear_pxn_table_bits_for_va (uint64 va, MMUParameters p, TableWalkCache cache, Cancellable? cancellable)
			...
			for (uint level = p.first_level; level != 3; level++) {
				...
				if ((raw_desc & TABLE_PXN_BIT) != 0) {
					...
					yield mdb.write_byte_array (slot_pa, d_buf.bytes, cancellable);
				}
		...
		public async Gee.List<uint64?> scan_ranges (Gee.List<Mumu.MemoryRange?> ranges, MatchPattern pattern, uint max_matches,
				Cancellable? cancellable) throws Error, IOError {
			...
			Mumu.ElfModule scanner;
			try {
				scanner = new Mumu.ElfModule.from_blob (raw_scanner);
			} catch (Mumu.Error e) {
			...
			bool was_running = mdb.state != STOPPED;
			if (was_running)
				yield mdb.stop (cancellable);

			GDB.Thread thread = mdb.exception.thread;
			...
			Bytes original_memory = yield mdb.read_byte_array (module_va, vm_size, cancellable);
			yield mdb.write_byte_array (module_va, scanner_module, cancellable);

			Bytes data = builder.build (data_va);
			yield mdb.write_byte_array (data_va, data, cancellable);
			...
			GDB.Breakpoint bp = yield mdb.add_breakpoint (SOFT, landing_zone_va, 4, cancellable);
			GDB.Exception ex = yield mdb.continue_until_exception (cancellable);
			if (ex.breakpoint != bp)
			...
			var pointer_size = mdb.pointer_size;
			var raw_matches = yield mdb.read_buffer (builder.address_of ("matches"), num_matches * pointer_size, cancellable);
			for (uint i = 0; i != num_matches; i++) {
				uint64 address = raw_matches.read_pointer (i * pointer_size);
				matches.add (address);
			}

			yield mdb.write_byte_array (module_va, original_memory, cancellable);
			yield module_allocation.deallocate (cancellable);
			yield thread.write_registers (saved_regs, cancellable);

			if (was_running)
				yield mdb.continue (cancellable);

			return matches;
		}

		public void apply_relocation (Mumu.ElfRelocationDetails r, uint64 base_va, Buffer relocated) throws Error {
			Mumu.ElfArm64Relocation type = (Mumu.ElfArm64Relocation) r.type;
			switch (type) {
				case ABS64:
				...
				default:
					throw new Error.NOT_SUPPORTED ("Unsupported relocation type: %s",
						Marshal.enum_to_nick<Mumu.ElfArm64Relocation> (type));
			}
		}
		...
		public async uint64 invoke (uint64 impl, uint64[] args, Cancellable? cancellable) throws Error, IOError {
			...
			GDB.Breakpoint bp = yield mdb.add_breakpoint (SOFT, landing_zone, 4, cancellable);
			GDB.Exception ex = null;
			do {
				ex = yield mdb.continue_until_exception (cancellable);
		...
		public async void execute (uint64 start, uint64 end, Cancellable? cancellable) throws Error, IOError {
			...
			GDB.Breakpoint bp = yield mdb.add_breakpoint (SOFT, end, 4, cancellable);
			GDB.Exception ex = null;
			do {
				ex = yield mdb.continue_until_exception (cancellable);
			} while (ex.breakpoint != bp || ex.thread.id != thread.id);
		...
		public async CallFrame load_call_frame (GDB.Thread thread, uint arity, Cancellable? cancellable) throws Error, IOError {
			...
			if (arity > NUM_ARGS_IN_REGS)
				stack = yield mdb.read_buffer (original_sp, (arity - NUM_ARGS_IN_REGS) * 8, cancellable);
		...
		public async InlineHook create_inline_hook (uint64 target, uint64 handler, Allocator allocator, Cancellable? cancellable)
			...
			var aw = new Mumu.Arm64Writer (scratch_buf);
			...
			aw.put_call_address_with_arguments (handler, 1,
				Mumu.ArgType.REGISTER, Mumu.Arm64Reg.SP);

			emit_epilog (aw, on_invoke_label);

			size_t redirect_size = aw.can_branch_directly_between (target, on_enter_trampoline) ? 4 : 16;

			var old_target_code = yield mdb.read_byte_array (target, redirect_size, cancellable);

			aw.put_label (on_invoke_label);
			var rl = new Mumu.Arm64Relocator (old_target_code.get_data (), aw);
			rl.input_pc = target;
			uint reloc_bytes = 0;
			do
			...
			yield mdb.write_byte_array (code_va, trampoline_code, cancellable);
			...
			return new Arm64InlineHook (target, old_target_code, new_target_code, allocation, mdb);
		}

		private static void emit_prolog (Mumu.Arm64Writer aw, uint64 target) {
			aw.put_sub_reg_reg_imm (SP, SP, 16);

			for (int i = 30; i != -2; i -= 2)
				aw.put_push_reg_reg (Mumu.Arm64Reg.Q0 + i, Mumu.Arm64Reg.Q1 + i);

			aw.put_push_reg_reg (FP, LR);
			for (int i = 27; i != -1; i -= 2)
				aw.put_push_reg_reg (Mumu.Arm64Reg.X0 + i, Mumu.Arm64Reg.X1 + i);
		...
		private static void emit_epilog (Mumu.Arm64Writer aw, void * next_hop_label) {
			...
			for (int i = 1; i != 29; i += 2)
				aw.put_pop_reg_reg (Mumu.Arm64Reg.X0 + i, Mumu.Arm64Reg.X1 + i);
			aw.put_pop_reg_reg (FP, LR);

			for (int i = 0; i != 32; i += 2)
				aw.put_pop_reg_reg (Mumu.Arm64Reg.Q0 + i, Mumu.Arm64Reg.Q1 + i);
		...
		private class Arm64InlineHook : Object, InlineHook {
			...
			public async void enable (Cancellable? cancellable) throws Error, IOError {
				...
				yield mdb.write_byte_array (target, new_target_code, cancellable);
				state = ENABLED;
			}
			...
			public async void disable (Cancellable? cancellable) throws Error, IOError {
				if (state != ENABLED)
					return;
				yield mdb.write_byte_array (target, old_target_code, cancellable);
				state = DISABLED;
			}
		}

		private static async void set_addressing_mode (GDB.Client mdb, AddressingMode mode, Cancellable? cancellable)
				throws Error, IOError {
			Gee.Set<string> features = mdb.features;
			if ("qemu-phy-mem-mode" in features)
				yield mdb.execute_simple ("Qqemu.PhyMemMode:" + ((mode == PHYSICAL) ? "1" : "0"), cancellable);
			else
		...
		private class MMUParameters {
			...
			public static async MMUParameters load (GDB.Client mdb, Cancellable? cancellable) throws Error, IOError {
				GDB.Exception? exception = mdb.exception;
				if (exception == null)
					throw new Error.INVALID_OPERATION ("Unable to query in current state");
				GDB.Thread thread = exception.thread;
		...
		private class DescriptorAllocation : Object, Allocation {
			...
			public async void deallocate (Cancellable? cancellable) throws Error, IOError {
				...
				yield set_addressing_mode (mdb, PHYSICAL, cancellable);
				try {
					yield mdb.write_byte_array (first_slot, d, cancellable);
				} finally {
					set_addressing_mode.begin (mdb, VIRTUAL, null);
				}
			}
		...
		private static Mumu.PageProtection protection_from_flags (DescriptorFlags flags, MMUParameters p) {
			if (p.sprr_enabled) {
				uint idx = 0;
		...
		private static uint64 apply_protection_bits (uint64 descriptor, Mumu.PageProtection prot, MMUParameters p) throws Error {
		...
		private static uint64 apply_non_sprr_protection_bits (uint64 descriptor, Mumu.PageProtection prot) {
			uint64 result = descriptor;

			bool want_exec = (prot & Mumu.PageProtection.EXECUTE) != 0;
			bool want_write = (prot & Mumu.PageProtection.WRITE) != 0;
		...
		private static uint find_sprr_index_for_permissions (Mumu.PageProtection prot, MMUParameters p) throws Error {
			bool want_read = (prot & Mumu.PageProtection.READ) != 0;
			bool want_write = (prot & Mumu.PageProtection.WRITE) != 0;
			bool want_exec = (prot & Mumu.PageProtection.EXECUTE) != 0;
		...
		private static Mumu.PageProtection protection_from_corellium_pt_details (string details) {
			Mumu.PageProtection prot = READ;
			if (!("pXN" in details))
				prot |= EXECUTE;
