...
namespace Miru.Barebone {
	public async Allocation inject_elf (Mumu.ElfModule elf, Bytes raw_elf, size_t page_size, Machine machine, Allocator allocator,
		...
		elf.enumerate_segments (s => {
			if ((s.protection & Mumu.PageProtection.EXECUTE) != 0) {
		...
		try {
			...
			yield machine.mdb.write_byte_array (base_va, relocated_image, cancellable);
