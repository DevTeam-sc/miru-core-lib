...
static void miru_collect_process_info_from_pid (guint pid, MiruEnumerateProcessesOperation * op);
static void miru_collect_process_info_from_kinfo (struct kinfo_proc * process, MiruEnumerateProcessesOperation * op);

static void miru_add_process_metadata (GHashTable * parameters, const struct kinfo_proc * process);

static struct kinfo_proc * miru_system_query_kinfo_procs (guint * count);
static gboolean miru_system_query_proc_pathname (pid_t pid, gchar * path, gsize size);
...
miru_system_enumerate_processes (MiruProcessQueryOptions * options, int * result_length)
{
  ...
  {
    miru_process_query_options_enumerate_selected_pids (options, (GFunc) miru_collect_process_info_from_pid, &op);
  }
  else
  {
    ...
    processes = miru_system_query_kinfo_procs (&count);

    for (i = 0; i != count; i++)
      miru_collect_process_info_from_kinfo (&processes[i], &op);
...
static void
miru_collect_process_info_from_pid (guint pid, MiruEnumerateProcessesOperation * op)
{
  ...
  miru_collect_process_info_from_kinfo (&process, op);
}

static void
miru_collect_process_info_from_kinfo (struct kinfo_proc * process, MiruEnumerateProcessesOperation * op)
{
  MiruHostProcessInfo info = { 0, };
  MiruScope scope = op->scope;
  ...
  if (scope != MIRU_SCOPE_MINIMAL)
    miru_add_process_metadata (info.parameters, process);

  still_alive = miru_system_query_proc_pathname (info.pid, path, sizeof (path));
  if (still_alive)
  {
    if (path[0] != '\0')
...
static void
miru_add_process_metadata (GHashTable * parameters, const struct kinfo_proc * process)
{
  const struct timeval * started = &process->ki_start;
  GDateTime * t0, * t1;

  g_hash_table_insert (parameters, g_strdup ("user"), miru_uid_to_name (process->ki_uid));
...
miru_system_query_kinfo_procs (guint * count)
{
...
static gboolean
miru_system_query_proc_pathname (pid_t pid, gchar * path, gsize size)
{
  gboolean success;
  int mib[4];
  size_t n;
