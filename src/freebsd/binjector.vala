namespace Miru {
	...
	public Mumu.Address _find_entrypoint (uint pid) throws Error {
		string program_path;
		try {
			program_path = Mumu.Freebsd.query_program_path_for_pid ((Posix.pid_t) pid);
		} catch (Mumu.Error e) {
			throw new Error.NOT_SUPPORTED ("Unable to detect entrypoint: %s", e.message);
		}

		Mumu.ElfModule? program_module = null;
		Mumu.Address program_base = 0;
		Mumu.Freebsd.enumerate_ranges ((Posix.pid_t) pid, READ, details => {
			unowned Mumu.FileMapping? file = details.file;
			if (file == null || file.offset != 0 || file.path != program_path)
				return true;

			try {
				program_module = new Mumu.ElfModule.from_file (file.path);
	...
	public string _detect_libthr_name () {
		string? name = null;
		Mumu.Freebsd.enumerate_ranges (Posix.getpid (), READ, details => {
			unowned Mumu.FileMapping? file = details.file;
	...
	protected class SymbolResolver : Object {
		...
		public SymbolResolver (uint pid) {
			Mumu.Freebsd.enumerate_ranges ((Posix.pid_t) pid, READ, details => {
				...
				Mumu.Address base_address = details.range.base_address;
		...
		public Mumu.Address find_ld_function (string function_name) {
			if (ld == null)
				return 0;
			return ld.resolve (function_name);
		}

		public Mumu.Address find_libc_function (string function_name) {
			if (libc == null)
				return 0;
	...
	private sealed class RemoteModule {
		private Mumu.Address base_address;
		private Mumu.ElfModule module;

		public static RemoteModule? try_open (Mumu.Address base_address, string path) {
			try {
				var module = new Mumu.ElfModule.from_file (path);
		...
		private RemoteModule (Mumu.Address base_address, Mumu.ElfModule module) {
			this.base_address = base_address;
			this.module = module;
		}

		public Mumu.Address resolve (string function_name) {
			Mumu.Address relative_address = 0;
