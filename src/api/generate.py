...
def main():
    parser = argparse.ArgumentParser(description="Generate refined Miru API definitions")
    parser.add_argument('--output', dest='output_type', choices=['bundle', 'header', 'gir', 'vapi', 'vapi-stamp'], default='bundle')
    parser.add_argument('miru_version', metavar='miru-version', type=str)
    parser.add_argument('miru_major_version', metavar='miru-major-version', type=str)
    parser.add_argument('miru_minor_version', metavar='miru-minor-version', type=str)
    parser.add_argument('miru_micro_version', metavar='miru-micro-version', type=str)
    parser.add_argument('miru_nano_version', metavar='miru-nano-version', type=str)
    parser.add_argument('api_version', metavar='api-version', type=str)
    parser.add_argument('core_header', metavar='/path/to/miru-core.h', type=argparse.FileType('r', encoding='utf-8'))
    parser.add_argument('core_gir', metavar='/path/to/Miru-x.y.gir', type=argparse.FileType('r', encoding='utf-8'))
    parser.add_argument('core_vapi', metavar='/path/to/miru-core.vapi', type=argparse.FileType('r', encoding='utf-8'))
    parser.add_argument('base_header', metavar='/path/to/miru-base.h', type=argparse.FileType('r', encoding='utf-8'))
    parser.add_argument('base_gir', metavar='/path/to/MiruBase-x.y.gir', type=argparse.FileType('r', encoding='utf-8'))
    parser.add_argument('base_vapi', metavar='/path/to/miru-base.vapi', type=argparse.FileType('r', encoding='utf-8'))
    parser.add_argument('output_dir', metavar='/output/dir')

    args = parser.parse_args()

    output_type = args.output_type
    miru_version = args.miru_version
    miru_version_components = (
        args.miru_major_version,
        args.miru_minor_version,
        args.miru_micro_version,
        args.miru_nano_version,
    )
    ...
    if output_type == 'vapi-stamp':
        (output_dir / f"miru-core-{api_version}.vapi.stamp").write_bytes(b"")
        return
    ...
    api = parse_api(miru_version, miru_version_components, api_version, toplevel_code, core_header, core_vapi, base_header, base_vapi)
...
def emit_header(api, output_dir):
    with OutputFile(output_dir / 'miru-core.h') as output_header_file:
        output_header_file.write("#ifndef __MIRU_CORE_H__\n#define __MIRU_CORE_H__\n\n")

        output_header_file.write("#include <glib.h>\n#include <glib-object.h>\n#include <gio/gio.h>\n#include <json-glib/json-glib.h>\n\n")

        output_header_file.write(f"#define MIRU_VERSION \"{api.miru_version}\"\n\n")

        for name, value in zip(['MAJOR', 'MINOR', 'MICRO', 'NANO'], api.miru_version_components):
            output_header_file.write(f"#define MIRU_{name}_VERSION {value}\n")

        output_header_file.write("""
#define MIRU_CHECK_VERSION(maj, min, mic) \\
    (MIRU_CURRENT_VERSION >= MIRU_VERSION_ENCODE ((maj), (min), (mic)))

#define MIRU_CURRENT_VERSION \\
    MIRU_VERSION_ENCODE (    \\
        MIRU_MAJOR_VERSION,  \\
        MIRU_MINOR_VERSION,  \\
        MIRU_MICRO_VERSION)

#define MIRU_VERSION_ENCODE(maj, min, mic) \\
...
""")
        ...
        output_header_file.write("\n\n/* Library lifetime */")
        output_header_file.write("\nvoid miru_init (void);")
        output_header_file.write("\nvoid miru_shutdown (void);")
        output_header_file.write("\nvoid miru_deinit (void);")
        output_header_file.write("\nGMainContext * miru_get_main_context (void);")

        output_header_file.write("\n\n/* Object lifetime */")
        output_header_file.write("\nvoid miru_unref (gpointer obj);")

        output_header_file.write("\n\n/* Library versioning */")
        output_header_file.write("\nvoid miru_version (guint * major, guint * minor, guint * micro, guint * nano);")
        output_header_file.write("\nconst gchar * miru_version_string (void);")
        ...
        if len(api.error_types) > 0:
            output_header_file.write("\n\n/* Errors */\n")
            output_header_file.write("\n\n".join(map(lambda enum: "GQuark miru_%(name_lc)s_quark (void);\n" \
        ...
        macros = []
        for enum in api.enum_types:
            macros.append("#define MIRU_TYPE_%(name_uc)s (miru_%(name_lc)s_get_type ())" \
                % { 'name_lc': enum.name_lc, 'name_uc': enum.name_uc })
        for object_type in api.object_types:
            macros.append("""#define MIRU_TYPE_%(name_uc)s (miru_%(name_lc)s_get_type ())
#define MIRU_%(name_uc)s(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), MIRU_TYPE_%(name_uc)s, Miru%(name)s))
#define MIRU_IS_%(name_uc)s(obj) (G_TYPE_CHECK_INSTANCE_TYPE ((obj), MIRU_TYPE_%(name_uc)s))""" \
                % { 'name': object_type.name, 'name_lc': object_type.name_lc, 'name_uc': object_type.name_uc })

        for enum in api.error_types:
            macros.append("#define MIRU_%(name_uc)s (miru_%(name_lc)s_quark ())" \
                % { 'name_lc': enum.name_lc, 'name_uc': enum.name_uc })
        output_header_file.write("\n" + "\n\n".join(macros))
...
def emit_gir(api: ApiSpec, core_gir: str, base_gir: str, output_dir: Path) -> str:
        ...
        for elem in core_elements + base_elements:
            if tag_name == "class":
                for child in list(elem):
                    if (child.tag == CORE_TAG_IMPLEMENTS and child.get("name") in {"Miru.HostSessionHub", "Miru.MiruMessageSink",
                                                                                   "MiruBase.MiruMessageSink"}) \
    ...
    result = result.replace("MiruBase.", "Miru.")
    with OutputFile(output_dir / f"Miru-{api.version}.gir") as output_gir:
...
def emit_vapi(api, output_dir):
    with OutputFile(output_dir / f"miru-core-{api.version}.vapi") as output_vapi_file:
        output_vapi_file.write("[CCode (cheader_filename = \"miru-core.h\", cprefix = \"Miru\", lower_case_cprefix = \"miru_\")]")
    ...
    with OutputFile(output_dir / f"miru-core-{api.version}.deps") as output_deps_file:
        output_deps_file.write("glib-2.0\n")
        output_deps_file.write("gobject-2.0\n")
        output_deps_file.write("gio-2.0\n")

def parse_api(miru_version, miru_version_components, api_version, toplevel_code, core_header, core_vapi, base_header, base_vapi):
    ...
    for line in (core_vapi + base_vapi).split("\n"):
        ...
        if level == 0:
            ...
            if stripped_line.startswith("public " + current_object_type.name + " (") or stripped_line.startswith("public static Miru." + current_object_type.name + " @new ("):
    ...
    for f in functions:
        m = re.search(r"^[\w\*]+ miru_{}.+?;".format(f.name), all_headers, re.MULTILINE | re.DOTALL)
        f.c_prototype = beautify_cprototype(m.group(0))

    return ApiSpec(miru_version, miru_version_components, api_version, object_types, functions, enum_types, error_types)
...
class ApiSpec:
    miru_version: str
    miru_version_components: List[int]
    version: str
