[CCode (gir_namespace = "MiruDroidyInjector", gir_version = "1.0")]
namespace Miru.Droidy.Injector {
	...
	private sealed class Session : Object {
		...
		private async GadgetDetails inject_gadget (Cancellable? cancellable) throws Error, IOError {
			...
			string so_path_shared = "/data/local/tmp/miru-gadget-" + instance_id + ".so";
			string so_path_app = "/data/data/" + package + "/gadget.so";
			string config_path_shared = "/data/local/tmp/miru-gadget-" + instance_id + ".config";
			string config_path_app = "/data/data/" + package + "/gadget.config";
			string unix_socket_path = "miru:" + package;
