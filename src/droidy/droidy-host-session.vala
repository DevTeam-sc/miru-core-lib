	...
	public sealed class DroidyHostSession : Object, HostSession {
		...
		public async uint spawn (string program, HostSpawnOptions options, Cancellable? cancellable) throws Error, IOError {
			...
			if (user_gadget_path != null) {
				...
				gadget_path = Path.build_filename (Environment.get_user_cache_dir (), "miru", "gadget-android-arm64.so");
			}
			...
			try {
				...
				if (e is IOError.CANCELLED) {
					...
					if (e is Error.SERVER_NOT_RUNNING) {
						api_error = new Error.SERVER_NOT_RUNNING ("Unable to connect to remote miru-server");
		...
		private async AndroidHelperClient get_helper_client (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				string helper_path = "/data/local/tmp/miru-helper-" + instance_id + ".dex";

				var helper_dex = new MemoryInputStream.from_bytes (
					new Bytes.static (Miru.Data.Android.get_helper_dex_blob ().data));
				...
				try {
					...
					yield client.request_protocol_change ("localabstract:/miru-helper-" + instance_id, cancellable);
				} catch (GLib.Error e) {
					client.close.begin ();
					throw e;
				}
