[CCode (gir_namespace = "MiruLLDB", gir_version = "1.0")]
namespace Miru.LLDB {
	...
	public enum ASLR {
		...
		public static ASLR from_nick (string nick) throws Miru.Error {
	...
	public sealed class Thread : GDB.Thread {
		...
		public async Gee.ArrayList<Frame> generate_backtrace (StackBounds? stack = null, Cancellable? cancellable = null) throws Error, IOError {
			...
			var mldb = (Client) client;

			var sp = yield read_register ("sp", cancellable);
			var lr = mldb.strip_code_address (yield read_register ("lr", cancellable));
			...
			while (current >= stack.bottom && current < stack.top && frame_pointer_is_aligned (current)) {
				...
				uint64 return_address = mldb.strip_code_address (frame.read_pointer (8));
	...
	public enum Signal {
		...
		public static Signal from_nick (string nick) throws Miru.Error {
	...
	public enum MachExceptionType {
		...
		public static MachExceptionType from_nick (string nick) throws Miru.Error {
