namespace Miru {
	...
	public sealed class FruityHostSessionProvider : Object, HostSessionProvider, HostChannelProvider, Pairable {
		...
		public async AgentSession link_agent_session (HostSession host_session, AgentSessionId id, MiruMessageSink sink,
	...
	public sealed class FruityHostSession : Object, HostSession {
		...
		private Gee.HashMap<uint, LLDBSession> mldb_sessions = new Gee.HashMap<uint, LLDBSession> ();
		...
		private const string GADGET_APP_ID = "re.miru.Gadget";
		...
		public async void close (Cancellable? cancellable) throws IOError {
			...
			while (!mldb_sessions.is_empty) {
				var iterator = mldb_sessions.values.iterator ();
				iterator.next ();
		...
		public async uint spawn (string program, HostSpawnOptions options, Cancellable? cancellable) throws Error, IOError {
			...
			var mldb = yield start_mldb_service (cancellable);
			var process = yield mldb.launch (argv, launch_options, cancellable);
			if (process.observed_state == ALREADY_RUNNING) {
				try {
					yield mldb.kill (cancellable);
				} catch (Error e) {
				}
				yield mldb.close (cancellable);

				mldb = yield start_mldb_service (cancellable);
				process = yield mldb.launch (argv, launch_options, cancellable);
			}

			var session = new LLDBSession (mldb, process, gadget_path, ExceptionHandlingStatus.LIMITED, device);
			add_mldb_session (session);
		...
		public async void kill (uint pid, Cancellable? cancellable) throws Error, IOError {
			var mldb_session = mldb_sessions[pid];
			if (mldb_session != null) {
				yield mldb_session.kill (cancellable);
				return;
			}
			...
			try {
				var mldb = yield start_mldb_service (cancellable);
				var process = yield mldb.attach_by_pid (pid, cancellable);

				mldb_session = new LLDBSession (mldb, process, null, ExceptionHandlingStatus.FULL, device);
				yield mldb_session.kill (cancellable);
				yield mldb_session.close (cancellable);
			} catch (Error e) {
		...
		public async AgentSessionId attach (uint pid, HashTable<string, Variant> options,
			...
			var mldb_session = mldb_sessions[pid];
			if (mldb_session != null) {
				var gadget_details = yield mldb_session.query_gadget_details (cancellable);
			...
			if (pid == 0)
				throw new Error.NOT_SUPPORTED ("The Miru system session is not available on jailed iOS");
			...
			mldb_session = new LLDBSession (mldb, process, gadget_path, ExceptionHandlingStatus.LIMITED, device);
			add_mldb_session (mldb_session);
		...
		private async LLDB.Client start_mldb_service (Cancellable? cancellable) throws Error, IOError {
			foreach (unowned string endpoint in DEBUGSERVER_ENDPOINT_CANDIDATES) {
				try {
					var mldb_stream = yield device.open_lockdown_service (endpoint, cancellable);
					return yield LLDB.Client.open (mldb_stream, cancellable);
				} catch (Error e) {
		...
		public async AgentSession link_agent_session (AgentSessionId id, MiruMessageSink sink,
				Cancellable? cancellable) throws Error, IOError {
			...
			entry.sink_registration_id = connection.register_object (ObjectPath.for_miru_message_sink (remote_id), sink);
		...
		private void add_mldb_session (LLDBSession session) {
			mldb_sessions[session.process.pid] = session;

			session.closed.connect (on_mldb_session_closed);
			session.output.connect (on_mldb_session_output);
		}

		private void remove_mldb_session (LLDBSession session) {
			mldb_sessions.unset (session.process.pid);

			session.closed.disconnect (on_mldb_session_closed);
			session.output.disconnect (on_mldb_session_output);
		}

		private void on_mldb_session_closed (LLDBSession session) {
			remove_mldb_session (session);
		}

		private void on_mldb_session_output (LLDBSession session, Bytes bytes) {
		...
		private async void detach_entry (GadgetEntry entry, SessionDetachReason reason) throws Error, IOError {
			...
			var mldb_session = mldb_sessions[entry.pid];
			if (mldb_session != null) {
				remove_mldb_session (mldb_session);
				if (reason == SessionDetachReason.APPLICATION_REQUESTED)
					yield entry.detach_mldb (mldb_session, io_cancellable);
				else
					yield mldb_session.close (io_cancellable);
			}
		...
		private async RemoteServer get_remote_server (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				if (connection.closed)
					throw new Error.SERVER_NOT_RUNNING ("Unable to connect to remote miru-server");
				...
				if (e is IOError.CANCELLED) {
					...
					if (e is Error) {
						...
						api_error = new Error.PROTOCOL ("Incompatible miru-server version");
					} else {
						api_error = new Error.SERVER_NOT_RUNNING ("Unable to connect to remote miru-server: %s",
							e.message);
					}
		...
		private class LLDBSession : Object {
			...
			public LLDBSession (LLDB.Client mldb, LLDB.Process process, string? gadget_path, ExceptionHandlingStatus ehs,
			...
			construct {
				mldb.closed.connect (on_mldb_closed);
				mldb.console_output.connect (on_mldb_console_output);
			}

			~LLDBSession () {
				detach_exception_handler ();

				mldb.closed.disconnect (on_mldb_closed);
				mldb.console_output.disconnect (on_mldb_console_output);
			}
			...
			private async void handle_exception (GDB.Exception exception) throws Error, IOError {
				var e = (Miru.LLDB.Exception) exception;
				...
				if (sig == SIGTRAP && e.metype == EXC_BREAKPOINT && medata.size == 2) {
					...
					if (x1 == x2 == 1337) {
						...
						switch (action) {
							case RESUME: {
								resumed = true;
								yield mldb.continue (io_cancellable);

								return;
							}
							case DETACH: {
								yield mldb.detach (io_cancellable);
								detached ();
							...
							case PAGE_PLAN: {
								...
								yield DebuggerMappings.handle_page_plan (mldb, plan_address, plan_size,
				...
				yield mldb.close (io_cancellable);
			}
			...
			private async void continue_gadget_threads (Cancellable? cancellable) throws Error, IOError {
				...
				yield mldb.enumerate_threads (thread => {
					if (thread.id != main_thread_id)
						gadget_threads.add (thread);
					return true;
				}, cancellable);
				yield mldb.continue_specific_threads (gadget_threads, cancellable);
			}
			...
			public async Fruity.Injector.GadgetDetails query_gadget_details (Cancellable? cancellable) throws Error, IOError {
				...
				try {
					...
					if (path == null) {
						path = Path.build_filename (Environment.get_user_cache_dir (), "miru", "gadget-ios.dylib");
					...
					var ptrauth_support = (process.cpu_subtype == ARM64E)
						? Mumu.PtrauthSupport.SUPPORTED
						: Mumu.PtrauthSupport.UNSUPPORTED;
					var module = new Mumu.DarwinModule.from_file (path, Mumu.CpuType.ARM64, ptrauth_support);

					Fruity.Injector.Transaction transaction =
						yield Fruity.Injector.inject ((owned) module, mldb, channel_provider, cancellable);
					Fruity.Injector.GadgetDetails gadget = transaction.gadget;
					main_thread_id = transaction.main_thread_id;

					exception_handler = mldb.notify["exception"].connect ((obj, pspec) => {
						var exception = mldb.exception;
						if (exception != null)
			...
			private void detach_exception_handler () {
				if (exception_handler != 0) {
					if (mldb.state != CLOSED)
						mldb.disconnect (exception_handler);
					exception_handler = 0;
				}
			}

			private void on_mldb_closed () {
				closed ();
			}

			private void on_mldb_console_output (Bytes bytes) {
				output (bytes);
			}
		}
		...
		private class GadgetEntry : Object {
			...
			public async void detach_mldb (LLDBSession mldb_session, Cancellable? cancellable) throws Error, IOError {
				...
				try {
					...
					mldb_session.detached.connect (on_mldb_session_detached);
					...
					mldb_session.detached.disconnect (on_mldb_session_detached);
					detach_request = null;
				}
			}
			...
			private void on_mldb_session_detached () {
