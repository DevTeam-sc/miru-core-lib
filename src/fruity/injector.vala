[CCode (gir_namespace = "MiruFruityInjector", gir_version = "1.0")]
namespace Miru.Fruity.Injector {
	public static async Transaction inject (owned Mumu.DarwinModule module, LLDB.Client mldb, HostChannelProvider channel_provider,
			Cancellable? cancellable) throws Error, IOError {
		var session = new Session (module, mldb, channel_provider);
	...
	public sealed class Transaction : Object {
		...
		private LLDB.Client mldb;

		internal Transaction (GadgetDetails gadget, bool is_early_instrumentation, string main_thread_id, LLDB.Client mldb) {
		...
		public async void commit (Cancellable? cancellable) throws Error, IOError {
			if (is_early_instrumentation) {
				...
				yield mldb.continue (cancellable);
			}
		}
	}
	...
	private sealed class Session : Object {
		public Mumu.DarwinModule module {
			get;
			construct;
		}
		...
		public Session (Mumu.DarwinModule module, LLDB.Client mldb, HostChannelProvider channel_provider) {
			Object (
				module: module,
				mldb: mldb,
		...
		public async Transaction run (Cancellable? cancellable) throws Error, IOError {
			try {
				...
				if (existing_gadget != null) {
					...
					return new Transaction (existing_gadget, is_early_instrumentation, main_thread.id, mldb);
				}
				...
				return new Transaction (gadget, is_early_instrumentation, main_thread.id, mldb);
			} catch (GLib.Error e) {
		...
		private async GadgetDetails? setup (Cancellable? cancellable) throws GLib.Error {
			...
			yield mldb.enumerate_threads (thread => {
				...
				MatchInfo info;
				if (/^miru-gadget-tcp-(\d+)$/.match (name, 0, out info)) {
			...
			jit_page = yield mldb.allocate (page_size, "rx", cancellable);
			scratch_page = yield mldb.allocate (page_size, "rw", cancellable);

			dyld_fields = yield mldb.get_apple_dyld_fields (ALLOW_CACHE, cancellable);
			libsystem_initialized = yield mldb.read_bool (dyld_fields.libsystem_initialized, cancellable);
			if (libsystem_initialized) {
				dyld_base = yield mldb.read_pointer (dyld_fields.dyld_load_address, cancellable);
			} else {
				yield mldb.enumerate_modules (m => {
		...
		private async void teardown (Cancellable? cancellable) throws GLib.Error {
			yield mldb.deallocate (scratch_page, cancellable);
			yield mldb.deallocate (jit_page, cancellable);
		...
		private async GadgetDetails inject_module (Cancellable? cancellable) throws GLib.Error {
			...
			module.base_address = yield mldb.allocate (module_size, "rx", cancellable);

			yield DebuggerMappings.spray_pages (mldb, module.base_address, (size_t) module.segments.index (0).vm_size,
				page_size, cancellable);

			unowned Mumu.DarwinModuleImage image = module.image;
			unowned uint8[] image_data = ((uint8[]) image.data)[0:image.size];
			var buffer = mldb.make_buffer (new Bytes.static (image_data));
		...
		private void collect_needed_symbols_and_threaded_items (Buffer buffer, SymbolQueryBuilder symbols_needed,
			...
			Mumu.Address slide = module.slide;
			bool have_threaded_items = false;
			GLib.Error? pending_error = null;

			Mumu.FoundDarwinBindFunc collect_bind = bind => {
				switch (bind.type) {
					case POINTER: {
		...
		private void perform_bind_operations (Buffer buffer, SymbolSet symbols) throws GLib.Error {
			...
			Mumu.FoundDarwinBindFunc perform_bind = bind => {
				unowned string module_name = module.get_dependency_by_ordinal (bind.library_ordinal);
				unowned string symbol_name = bind.symbol_name;

				uint64 address;
				if (!symbols.lookup (module_name, symbol_name, out address)) {
					bool is_weak = (bind.symbol_flags & Mumu.DarwinBindSymbolFlags.WEAK_IMPORT) != 0;
		...
		private async uint16 upload (Buffer buffer, ThreadedItems threaded_items, ChainedFixups chained_fixups,
			...
			yield mldb.write_byte_array (code, new Bytes.static (UPLOAD_LISTENER_CODE), cancellable);

			const uint64 rx_buffer_size = 1024 * 1024;

			uint64 args = scratch_page;
			var args_builder = mldb.make_buffer_builder ();

			string range_param = ("miru_dylib_range=0x%" + uint64.FORMAT_MODIFIER + "x,0x%" + size_t.FORMAT_MODIFIER + "x")
			...
			string config_param = "miru_gadget_config=" + Base64.encode (raw_config.data);
			...
			yield mldb.write_byte_array (args, args_builder.build (), cancellable);
			...
			yield mldb.write_byte_array (code, new Bytes.static (UPLOAD_RECEIVER_CODE), cancellable);
		...
		private async void perform_upload (Buffer buffer, ThreadedItems threaded_items, ChainedFixups chained_fixups,
			...
			try {
				...
				var hello = mldb.make_buffer_builder ()
				...
				foreach (unowned Mumu.DarwinSegment segment in module.segments.data) {
					...
					var write_command_header = mldb.make_buffer_builder ()
				...
				if (!threaded_items.is_empty) {
					var command = mldb.make_buffer_builder ()
				...
				foreach (uint64 fixups_header_address in chained_fixups.locations) {
					var fixup_command = mldb.make_buffer_builder ()
				...
				foreach (unowned Mumu.DarwinSegment segment in module.segments.data) {
					...
					var protect_command = mldb.make_buffer_builder ()
				...
				var construct_commands = mldb.make_buffer_builder ();
				...
				var check_command = mldb.make_buffer_builder ()
		...
		private size_t compute_virtual_size (Mumu.DarwinModule module) {
			size_t total = 0;
		...
		private async void ensure_libsystem_initialized (Cancellable? cancellable) throws GLib.Error {
			...
			yield mldb.write_bool (dyld_fields.libsystem_initialized, true, cancellable);
		...
		private async void ensure_libsystem_initialized_for_dyld_v4_and_above (uint64 libdyld_initialize,
			...
			GDB.Breakpoint init_breakpoint = yield mldb.add_breakpoint (SOFT, initializer, 4, cancellable);
			...
			if (restart_with_dyld_in_cache != null)
				restart_breakpoint = yield mldb.add_breakpoint (SOFT, restart_with_dyld_in_cache, 4, cancellable);

			var exception = (LLDB.Exception) yield mldb.continue_until_exception (cancellable);
			...
			if (hit_breakpoint == restart_breakpoint) {
				...
				uint64 process_info = yield mldb.read_pointer (process_info_ptr, cancellable);
				uint64 dyld_image_load_address = yield mldb.read_pointer (process_info + 0x20, cancellable);
				...
				init_breakpoint = yield mldb.add_breakpoint (SOFT, initializer, 4, cancellable);

				exception = (LLDB.Exception) yield mldb.continue_until_exception (cancellable);
				...
				dyld_fields = yield mldb.get_apple_dyld_fields (BYPASS_CACHE, cancellable);
			}
			...
			uint64 frame_above = yield mldb.read_pointer (frame_here, cancellable);
			uint64 libsystem_initializer_caller = 0;
			bool falls_within_dyld = false;

			do {
				libsystem_initializer_caller = yield mldb.read_pointer (frame_above + 8, cancellable);
				libsystem_initializer_caller = mldb.strip_code_address (libsystem_initializer_caller);
				...
				if (!falls_within_dyld)
					frame_above = yield mldb.read_pointer (frame_above, cancellable);
			} while (!falls_within_dyld);

			GDB.Breakpoint caller_breakpoint = yield mldb.add_breakpoint (SOFT, libsystem_initializer_caller, 4, cancellable);
		...
		private async void ensure_libsystem_initialized_for_dyld_v3_and_below (Gee.Map<string, uint64?> dyld_symbols,
			...
			if (launch_with_closure != 0) {
				...
				modern_breakpoint = yield mldb.add_breakpoint (SOFT, run_initializers_call, 4, cancellable);
			}
			...
			GDB.Breakpoint legacy_breakpoint = yield mldb.add_breakpoint (SOFT, initialize_main_executable, 4, cancellable);

			var exception = yield mldb.continue_until_exception (cancellable);
		...
		private async uint64 find_dyld3_run_initializers_call (uint64 launch_with_closure, Cancellable? cancellable)
			...
			var buffer = yield mldb.read_buffer (launch_with_closure, max_size, cancellable);
		...
		private async void initialize_libsystem_from_legacy_codepath (Gee.Map<string, uint64?> dyld_symbols,
			...
			var code_builder_pac = mldb.make_buffer_builder ();
			...
			yield mldb.write_byte_array (code, code_builder_pac.build (), cancellable);

			var code_builder = mldb.make_buffer_builder ();
			...
			var helpers_builder = mldb.make_buffer_builder ();
			...
			yield mldb.write_byte_array (helpers, helpers_buf, cancellable);
			...
			yield mldb.write_byte_array (code, code_builder.build (), cancellable);
			...
			var strcmp_breakpoint = yield mldb.add_breakpoint (SOFT, strcmp_impl, 4, cancellable);
			*/
		...
		private async Gee.Map<string, uint64?> fetch_dyld_symbols (Cancellable? cancellable) throws GLib.Error {
			...
			uint64 code = jit_page;
			yield mldb.write_byte_array (code, new Bytes.static (SYMBOL_FETCHER_CODE), cancellable);
			...
			Bytes output_bytes = yield mldb.read_byte_array (output, size, cancellable);
		...
		private async SymbolSet resolve_symbols (SymbolQuery query, Cancellable? cancellable) throws GLib.Error {
			uint64 code = jit_page;
			yield mldb.write_byte_array (code, new Bytes.static (SYMBOL_RESOLVER_CODE), cancellable);

			var state_builder = mldb.make_buffer_builder ();
			...
			if (state_size <= page_size)
				...
				state = yield mldb.allocate (state_size, "rw", cancellable);
			strv_builder.build (state);
			yield mldb.write_byte_array (state, state_builder.build (), cancellable);
			...
			var output_vector = yield mldb.read_buffer (output_vector_address, output_vector_size, cancellable);
			size_t offset = 0;
			foreach (var group in query.groups) {
				...
				foreach (var symbol_name in group.symbol_names) {
					result_group[symbol_name] = mldb.strip_code_address (output_vector.read_pointer (offset));
			...
			if (state != scratch_page)
				yield mldb.deallocate (state, cancellable);
		...
		private class ChainedFixupsBuilder {
			...
			public unowned ChainedFixupsBuilder add_location (Mumu.Address vm_address) {
			...
			while (true) {
				...
				if ((LLDB.Signal) exception.signum == SIGTRAP && exception.metype == EXC_BREAKPOINT) {
					...
					if (x1 == x2 == 1337 && action == PAGE_PLAN) {
						...
						yield DebuggerMappings.handle_page_plan (mldb, plan_address, plan_size, page_size,
							cancellable);
		...
		/*
		private async void dump_mldb_state (Cancellable? cancellable) throws GLib.Error {
			...
			yield mldb.enumerate_threads (t => {
				threads.add (t);
				return true;
			}, cancellable);
