[CCode (gir_namespace = "MiruFruity", gir_version = "1.0")]
namespace Miru.DebuggerMappings {
	internal async void spray_pages (GDB.Client mdb, uint64 address, size_t size, size_t page_size, Cancellable? cancellable)
			throws Error, IOError {
		int n_pages = (int) (size / page_size);
		if (n_pages == 0)
			throw new Error.INVALID_ARGUMENT ("Expected one or more pages");

		var builder = mdb.make_packet_builder_sized (64);
		...
		for (int i = 0; i < n_pages; i += 2) {
			...
			mdb.perform_execute.begin (builder.build (), cancellable, request);
	...
	internal async void handle_page_plan (GDB.Client mdb, uint64 address, size_t size, size_t page_size, Cancellable? cancellable)
			throws Error, IOError {
		var plan = new BufferReader (yield mdb.read_buffer (address, size, cancellable));
