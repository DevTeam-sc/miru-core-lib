...
typedef struct _MiruLibdyldApi MiruLibdyldApi;
typedef struct _MiruMachO MiruMachO;

struct _MiruLibdyldApi
{
...
static void miru_get_libdyld_api (const struct dyld_all_image_infos * all_image_info, MiruLibdyldApi * api);
static void miru_parse_macho (const void * macho, MiruMachO * result);
static const void * miru_find_libdyld (const struct dyld_all_image_infos * all_image_info);
static uint64_t miru_exports_trie_find (const uint8_t * exports, const char * name);
uint64_t miru_read_uleb128 (const uint8_t ** data);

static bool miru_str_equals (const char * str, const char * other);

void
miru_resolve_symbols (const char ** input_vector, void ** output_vector, const struct dyld_all_image_infos * all_image_info)
{
  MiruLibdyldApi api;
  const char ** input;
  void ** output;
  ...
  miru_get_libdyld_api (all_image_info, &api);
...
static void
miru_get_libdyld_api (const struct dyld_all_image_infos * all_image_info, MiruLibdyldApi * api)
{
  MiruMachO libdyld;

  miru_parse_macho (miru_find_libdyld (all_image_info), &libdyld);

  api->dlopen = libdyld.base + miru_exports_trie_find (libdyld.exports, "_dlopen");
  api->dlsym = libdyld.base + miru_exports_trie_find (libdyld.exports, "_dlsym");
}

static const void *
miru_find_libdyld (const struct dyld_all_image_infos * all_image_info)
{
  uint32_t i;
  ...
  for (i = 0; i != all_image_info->infoArrayCount; i++)
  {
    ...
    if (miru_str_equals (image->imageFilePath, "/usr/lib/system/libdyld.dylib"))
    {
...
static uint64_t
miru_exports_trie_find (const uint8_t * exports, const char * name)
{
  const char * s;
  const uint8_t * p;
  ...
  while (p != NULL)
  {
    ...
    terminal_size = miru_read_uleb128 (&p);
    ...
    for (i = 0; i != child_count; i++)
    {
      ...
      if (matching_edge)
      {
        node_offset = miru_read_uleb128 (&p);
        s = symbol_cur;
        break;
      }
      else
...
uint64_t
miru_read_uleb128 (const uint8_t ** data)
{
...
main (void)
{
  ...
  miru_resolve_symbols (input_vector, output_vector, dyld_info);
