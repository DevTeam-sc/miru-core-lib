...
def make(sourcedir: Path,
    ...
    test_options = shlex.split(environ.get("MIRU_TEST_OPTIONS", "-v"))
    ...
    env_state = pickle.loads((builddir / "miru-env.dat").read_bytes())

    machine_config = env_state["host"]
    if machine_config is None:
        machine_config = env_state["build"]
    meson_env = machine_config.make_merged_environment(environ)
    meson_env["MIRU_ALLOWED_PREBUILDS"] = ",".join(env_state["allowed_prebuilds"])
    meson_env["MIRU_DEPS"] = str(env_state["deps"])
