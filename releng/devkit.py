...
DEVKITS = {
    "miru-mumu": ("miru-mumu-1.0", Path("mumu") / "mumu.h"),
    "miru-mumujs": ("miru-mumujs-1.0", Path("mumujs") / "mumuscriptbackend.h"),
    "miru-core": ("miru-core-1.0", Path("miru-core.h")),
}
...
class CompilerApplication:
    ...
    def _generate_gir(self):
        if self.kit != "miru-core":
            return []

        gir_path = Path(query_pkgconfig_variable("miru_girdir", self.package, self.meson_config)) / "Miru-1.0.gir"
        gir_name = "miru-core.gir"
    ...
    def _generate_header(self, umbrella_header_path, thirdparty_symbol_mappings):
        ...
        if self.compiler_argument_syntax == "msvc":
            ...
            miru_root_slashed = REPO_ROOT.as_posix()
            header_files = [Path(h) for h in header_files if bool(re.match("^" + miru_root_slashed, h, re.I))]
        else:
        ...
        if kit in {"miru-mumu", "miru-mumujs"} and machine.os == "none":
            mumu_dir = umbrella_header_path.parent
            if kit == "miru-mumujs":
                mumu_dir = mumu_dir.parent.parent / "mumu"
            barebone_header = mumu_dir / "backend-barebone" / "include" / "mumu" / "mumubarebone.h"
            ingest_header(barebone_header, header_files, processed_header_files, devkit_header_lines)
        if kit == "miru-mumujs" and machine.os != "none":
            inspector_server_header = umbrella_header_path.parent / "mumuinspectorserver.h"
            ingest_header(inspector_server_header, header_files, processed_header_files, devkit_header_lines)
        if kit == "miru-core" and machine.os == "android":
            selinux_header = umbrella_header_path.parent / "miru-selinux.h"
        ...
        if package.startswith("miru-mumu"):
            config = """#ifndef MUMU_STATIC
# define MUMU_STATIC
#endif
...
"""
        ...
        if machine.os == "windows":
            ...
            if package == "miru-core-1.0":
                deps.extend(["advapi32", "crypt32", "gdi32", "kernel32", "ole32", "secur32", "shell32", "user32"])
            deps.sort()

            miru_pragmas = f"#pragma comment(lib, \"{compute_library_filename(kit, self.compiler_argument_syntax)}\")"
        ...
        if len(thirdparty_symbol_mappings) > 0:
            ...
            config += "#ifndef __MIRU_SYMBOL_MAPPINGS__\n"
            config += "#define __MIRU_SYMBOL_MAPPINGS__\n\n"
    ...
    def _generate_example(self, source_file, extra_ldflags):
        ...
        if machine.os == "windows":
            ...
            if machine.is_apple or machine.os == "android":
                cc = "clang++" if kit == "miru-mumujs" else "clang"
            else:
                cc = "g++" if kit == "miru-mumujs" else "gcc"
            meson_config = self.meson_config
...
def get_thirdparty_symbol_mappings(library, meson_config):
    return [(name, "_miru_" + name) for name in get_thirdparty_symbol_names(library, meson_config)]
...
def get_thirdparty_symbol_names(library, meson_config):
    ...
    miru_prefixes = ["miru", "_miru", "mumu", "_mumu"]
    thirdparty_names = [name for name in visible_names if not any([name.startswith(prefix) for prefix in miru_prefixes])]
