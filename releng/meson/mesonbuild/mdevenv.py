...
def add_mdb_auto_load(autoload_path: Path, mdb_helper: str, fname: Path) -> None:
    # Copy or symlink the GDB helper into our private directory tree
    destdir = autoload_path / fname.parent
    destdir.mkdir(parents=True, exist_ok=True)
    try:
        if is_windows():
            shutil.copy(mdb_helper, str(destdir / os.path.basename(mdb_helper)))
        else:
            os.symlink(mdb_helper, str(destdir / os.path.basename(mdb_helper)))
    except (FileExistsError, shutil.SameFileError):
        pass

def write_mdb_script(privatedir: Path, install_data: 'InstallData', workdir: Path) -> None:
    if not shutil.which('mdb'):
        return
    bdir = privatedir.parent
    autoload_basedir = privatedir / 'mdb-auto-load'
    autoload_path = Path(autoload_basedir, *bdir.parts[1:])
    have_mdb_helpers = False
    for d in install_data.data:
        if d.path.endswith('-mdb.py') or d.path.endswith('-mdb.mdb') or d.path.endswith('-mdb.scm'):
            ...
            for t in install_data.targets:
                ...
                if path.name == libname:
                    add_mdb_auto_load(autoload_path, d.path, path)
                    have_mdb_helpers = True
    if have_mdb_helpers:
        mdbinit_line = f'add-auto-load-scripts-directory {autoload_basedir}\n'
        mdbinit_path = bdir / '.mdbinit'
        first_time = False
        try:
            with mdbinit_path.open('r+', encoding='utf-8') as f:
                if mdbinit_line not in f.readlines():
                    f.write(mdbinit_line)
                    first_time = True
        except FileNotFoundError:
            mdbinit_path.write_text(mdbinit_line, encoding='utf-8')
            first_time = True
        if first_time:
            mdbinit_path = mdbinit_path.resolve()
            workdir_path = workdir.resolve()
            rel_path = mdbinit_path.relative_to(workdir_path)
            ...
            mlog.log(' - Add', mlog.bold(f'add-auto-load-safe-path {mdbinit_path.parent}'),
                     'in', mlog.bold('~/.mdbinit'))
            if mdbinit_path.parent != workdir_path:
...
def run(options: argparse.Namespace) -> int:
    ...
    write_mdb_script(privatedir, install_data, workdir)
