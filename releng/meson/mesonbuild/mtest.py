...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--mdb', default=False, dest='mdb', action='store_true',
                        help='Run test under mdb.')
    parser.add_argument('--mdb-path', default='mdb', dest='mdb_path',
                        help='Path to the mdb binary (default: mdb).')
...
class SingleTestRunner:
    ...
    def __init__(self, test: TestSerialisation, env: T.Dict[str, str], name: str,
        ...
        if self.options.mdb or self.test.timeout is None or self.test.timeout <= 0:
            timeout = None
        ...
        is_parallel = test.is_parallel and self.options.num_processes > 1 and not self.options.mdb
    ...
    async def _run_subprocess(self, args: T.List[str], *,
        ...
        # Let mdb handle ^C instead of us
        if self.options.mdb:
            previous_sigint_handler = signal.getsignal(signal.SIGINT)
            # Make the meson executable ignore SIGINT while mdb is running.
        ...
        def preexec_fn() -> None:
            if self.options.mdb:
                ...
                # We don't want setsid() in mdb because mdb needs the
...
class TestHarness:
    def __init__(self, options: argparse.Namespace):
        ...
        if self.options.logbase and not self.options.mdb:
            namebase = None
    ...
    def get_wrapper(options: argparse.Namespace) -> T.List[str]:
        wrap: T.List[str] = []
        if options.mdb:
            wrap = [options.mdb_path, '--quiet']
            if options.repeat > 1:
                wrap += ['-ex', 'run', '-ex', 'quit']
            # Signal the end of arguments to mdb
            wrap += ['--args']
        if options.wrapper:
...
def run(options: argparse.Namespace) -> int:
    ...
    if options.mdb:
        options.verbose = True
        if options.wrapper:
            print('Must not specify both a wrapper and mdb at the same time.')
            return 1
        check_bin = 'mdb'
