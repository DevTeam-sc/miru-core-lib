...
```console
$ meson test --mdb testname
```

Meson will launch `mdb` all set up to run the test. Just type `run` in
...
```console
$ meson test --mdb --repeat=10000 testname
```
...
will not kill `mdb` while the developer is still debugging it. The
...
```console
$ meson test --mdb --mdb-path /path/to/mdb testname
```
