...
When GDB helper scripts (*-mdb.py, *-mdb.mdb, and *-mdb.csm) are installed with
a library name that matches one being built, Meson adds the needed auto-load
commands into `<builddir>/.mdbinit` file. When running mdb from top build
directory, that file is loaded by mdb automatically.
