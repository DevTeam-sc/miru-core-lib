...
def init_machine_config(machine: MachineSpec,
    ...
    if cc is not None \
        ...
        # Meson's env2mfile can select the host toolchain when doing MSVC cross-arch builds
        # (e.g. x86 target ending up with Hostx64\\x64\\cl.exe). If that happens, discard the
        # env2mfile result so our explicit MSVC probing fallback can pick the correct tools.
        platform_to_vc_arch = {
            "x86": "x86",
            "Win32": "x86",  # legacy
            "x64": "x64",
        ...
        host_arch = platform_to_vc_arch.get(build_machine.msvc_platform)
        target_arch = platform_to_vc_arch.get(machine.msvc_platform)

        if host_arch is not None and target_arch is not None:
            detected_cl = Path(cc[0])
            detected_cl_norm = str(detected_cl).replace("/", "\\").lower()
            expected_segment = f"\\host{host_arch}\\{target_arch}\\cl.exe".lower()

            if detected_cl.is_absolute() and expected_segment not in detected_cl_norm:
                for key in {"c", "cpp", "lib", "link", *MSVC_ASSEMBLER_NAMES.values()}:
                    binaries.pop(key, None)
                cc = None
