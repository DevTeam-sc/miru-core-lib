...
submodules="miru-mumu miru-core miru-python miru-node miru-qml miru-clr"
...
summarize_repo_changes miru $from $to internal

for module in miru-mumu miru-core miru-python miru-node miru-qml miru-clr; do
  mod_range=${from}..${to}
  pushd subprojects/$module > /dev/null
  append_log $mod_range
  summarize_repo_changes $module $from $to internal
  if [ $module == miru-mumu ]; then
    git diff $mod_range -- bindings/mumujs/generate-runtime.py > "$intdir/bridge-changes"
...
bumped_deps=$(grep "_version = " "$scratch" \
    ...
    | cut -c 2- \
    | awk '{ print $1 }' \
    | grep -Ev "miru_(deps|bootstrap)_version" \
    | sed -e 's,_version$,,')
