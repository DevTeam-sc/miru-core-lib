...
def query_machine_file_path(machine: MachineSpec, flavor: str, build_dir: Path) -> Path:
    return build_dir / f"miru{flavor}-{machine.identifier}.txt"


def detect_default_prefix() -> Path:
    if platform.system() == "Windows":
        return Path(os.environ["ProgramFiles"]) / "Miru"
...
def generate_machine_config(machine: MachineSpec,
    ...
    if toolchain_prefix is not None:
        ...
        for (tool_name, filename_suffix) in {("mdbus-codegen", ""),
        ...
        if pkg_config_binary is not None:
            ...
            if sdk_prefix is not None:
                pkg_config += [f"--define-variable=miru_sdk_prefix={sdk_prefix}"]
    ...
    if vala_compiler is not None:
        ...
        if pkg_config is not None:
            wrapper = outdir / "miru-pkg-config.py"
    ...
    machine_file = outdir / f"miru-{machine.identifier}.txt"
...
def can_run_host_binaries(build_machine: MachineSpec,
    ...
    return environ.get("MIRU_CAN_RUN_HOST_BINARIES", "no") == "yes"
...
def find_exe_wrapper(machine: MachineSpec,
    ...
    qemu_sysroot = environ.get("MIRU_QEMU_SYSROOT")
    ...
    if qemu_binary is None:
        raise QEMUNotFoundError(f"unable to find {qemu_flavor}, needed due to MIRU_QEMU_SYSROOT being set")
