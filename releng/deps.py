...
def detect_cache_dir(sourcedir: Path) -> Path:
    raw_location = os.environ.get("MIRU_DEPS", None)
...
def sync(bundle: Bundle,
    ...
    try:
        ...
        suffix_len = len(".miru.in")
        raw_location = location.as_posix()
        for f in staging_dir.rglob("*.miru.in"):
            target = f.parent / f.name[:-suffix_len]
            f.write_text(f.read_text(encoding="utf-8").replace("@MIRU_TOOLROOT@", raw_location),
                         encoding="utf-8")
            f.rename(target)
...
def roll(bundle: Bundle,
    ...
    s3_url = "s3://build.miru.re/deps/{version}/{filename}".format(version=version, filename=filename)
...
class Builder:
    ...
    def _package(self):
        ...
        with tempfile.TemporaryDirectory(prefix="miru-deps") as raw_tempdir:
    ...
    def _adjust_files_containing_hardcoded_paths(self, bundledir: Path):
        ...
        for f in self._walk_plain_files(bundledir):
            ...
            try:
                ...
                replacement = "${miru_sdk_prefix}" if is_pcfile else "@MIRU_TOOLROOT@"
                ...
                if new_text != text:
                    ...
                    if not is_pcfile:
                        filepath.rename(filepath.parent / f"{f.name}.miru.in")
    ...
    def _file_is_toolchain_related(self, f: Path) -> bool:
        ...
        if parts[0] == "bin":
            if f.suffix == ".pdb":
                return False
            stem = f.stem
            if stem in {"mdbus", "gio", "gobject-query", "gsettings"}:
                return False
...
def bump_wraps(identifier: str,
    ...
    run(["git", "clone", "--depth", "1", f"git@github.com:miru/{identifier}.git"], cwd=workdir)
