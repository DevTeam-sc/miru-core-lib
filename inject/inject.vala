namespace Miru.Inject {
	...
	private sealed class ScriptRunner : Object, RpcPeer {
		...
		private async TerminalMode query_terminal_mode () {
			...
			try {
				mode_value = yield rpc_client.call ("getMiruTerminalMode", new Json.Node[] {}, null, io_cancellable);
			} catch (GLib.Error e) {
				return COOKED;
			}
		...
		public void on_stdin (string str, Bytes? data) {
			...
			rpc_client.call.begin ("onMiruStdin", new Json.Node[] { str_value }, data, io_cancellable);
		}
		...
		private void on_script_file_changed (File file, File? other_file, FileMonitorEvent event_type) {
			...
			source.attach (Miru.get_main_context ());
		...
		/**
		 * The script can send strings to miru-inject to write to its stdout or
		 * stderr. This can be done either inside the RPC handler for receiving
		 * input from miru-inject, or elsewhere at any arbitrary point in the
		 * script. We use the following syntax:
		 *
		 * send(['miru:stdout', 'DATA']);
		 * send(['miru:stderr', 'DATA']);
		 *
		 ...
		 *
		 * {"type":"send","payload":["miru:stdout","DATA"]}
		 */
