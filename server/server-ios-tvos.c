...
#define MIRU_TRUST_CACHE_INJECT_PATH "/usr/bin/inject"
...
static gboolean miru_is_platformized (void);
static gboolean miru_try_platformize (const gchar * path);

static gchar * miru_get_executable_path (void);
static gboolean miru_refresh_inode (const gchar * path);
static gboolean miru_add_to_trust_cache (const gchar * path);

void
_miru_server_ios_tvos_configure (void)
{
  memorystatus_control (MEMORYSTATUS_CMD_SET_JETSAM_TASK_LIMIT, getpid (), 256, NULL, 0);

  if (!miru_is_platformized ())
  {
    gchar * server_path;

    server_path = miru_get_executable_path ();

    if (miru_try_platformize (server_path))
    {
      g_print (
          "***\n"
...
miru_is_platformized (void)
{
  ...
  system_has_guarded_ports = mumu_darwin_check_xnu_version (7938, 0, 0);
...
static gboolean
miru_try_platformize (const gchar * path)
{
  if (!miru_add_to_trust_cache (path))
    return FALSE;

  if (!miru_refresh_inode (path))
    return FALSE;
...
static gboolean
miru_refresh_inode (const gchar * path)
{
...
static gboolean
miru_add_to_trust_cache (const gchar * path)
{
  GSubprocess * process;
  GError * error;

  if (!g_file_test (MIRU_TRUST_CACHE_INJECT_PATH, G_FILE_TEST_EXISTS))
    return FALSE;

  error = NULL;
  process = g_subprocess_new (G_SUBPROCESS_FLAGS_STDOUT_SILENCE | G_SUBPROCESS_FLAGS_STDERR_SILENCE,
      &error, MIRU_TRUST_CACHE_INJECT_PATH, path, NULL);
