...
static void miru_server_on_log_message (const gchar * log_domain, GLogLevelFlags log_level, const gchar * message, gpointer user_data);

static gboolean miru_verbose_logging_enabled = FALSE;

void
miru_server_environment_init (void)
{
  miru_init_with_runtime (MIRU_RUNTIME_GLIB);

  g_log_set_default_handler (miru_server_on_log_message, NULL);
}

void
miru_server_environment_set_verbose_logging_enabled (gboolean enabled)
{
  miru_verbose_logging_enabled = enabled;
}

void
miru_server_environment_configure (void)
{
#if defined (HAVE_IOS) || defined (HAVE_TVOS) || defined (HAVE_XROS)
  _miru_server_ios_tvos_configure ();
#endif
...
static void
miru_server_on_log_message (const gchar * log_domain, GLogLevelFlags log_level, const gchar * message, gpointer user_data)
{
  if (!miru_verbose_logging_enabled && (log_level & G_LOG_LEVEL_MASK) >= G_LOG_LEVEL_DEBUG)
    return;
