namespace Miru.Agent {
	public void main (string agent_parameters, ref Miru.UnloadPolicy unload_policy, void * injector_state) {
	...
	private sealed class Runner : Object, ProcessInvader, AgentSessionProvider, ExitHandler, ForkHandler, SpawnHandler {
		...
		private static Mumu.MemoryRange cached_agent_range;
		...
		private Mumu.ThreadId agent_tid;
		...
		private Mumu.MemoryRange agent_range;
		...
		public static void create_and_run (string agent_parameters, ref Miru.UnloadPolicy unload_policy,
				void * opaque_injector_state) {
			Environment._init ();

			{
				Mumu.MemoryRange? mapped_range = null;
				...
				if (cached_agent_path == null) {
					...
					Mumu.Cloak.add_range (cached_agent_range);
				}
				...
				if (injector_state != null) {
					...
					Mumu.Cloak.add_file_descriptor (injector_state.fifo_fd);
				}
#endif
				...
				var ignore_scope = new ThreadIgnoreScope (MIRU_THREAD);
...
#if LINUX || FREEBSD
					if (injector_state != null)
						Mumu.Cloak.remove_file_descriptor (injector_state.fifo_fd);
#endif
		...
		public static void resume_after_transition (ref Miru.UnloadPolicy unload_policy, void * opaque_injector_state) {
			{
#if LINUX || FREEBSD
				var injector_state = (PosixInjectorState *) opaque_injector_state;
				if (injector_state != null) {
		...
		private Runner (string agent_parameters, string? agent_path, Mumu.MemoryRange agent_range) {
		...
		construct {
			agent_tid = Mumu.Process.get_current_thread_id ();
		...
		private async void start (owned FileDescriptorTablePadder padder) {
...
#if DARWIN
			enable_exceptor = !Mumu.Darwin.query_hardened ();
#endif
...

			{
				...
				this.exceptor = Mumu.Exceptor.obtain ();
			...
			Mumu.ScriptBackend.get_scheduler ().push_job_on_js_thread (Priority.DEFAULT, () => {
		...
		private void keep_running_eternalized () {
			agent_gthread = new Thread<bool> ("miru-eternal-agent", () => {
				var ignore_scope = new ThreadIgnoreScope (MIRU_THREAD);
		...
		private bool supports_async_exit () {
			...
			return Mumu.Process.has_thread (agent_tid);
		}
...
#if !WINDOWS
			MumuJS.prepare_to_fork ();
			Mumu.prepare_to_fork ();
		...
		private static void resume_subsystems () {
			...
			Mumu.recover_from_fork_in_parent ();
			MumuJS.recover_from_fork_in_parent ();
#endif
		}
		...
		private static void resume_subsystems_in_child () {
			...
			Mumu.recover_from_fork_in_child ();
			MumuJS.recover_from_fork_in_child ();
#endif
		}
		...
		public Mumu.MemoryRange get_memory_range () {
			return agent_range;
		}

		public Mumu.ScriptBackend get_script_backend (ScriptRuntime runtime) throws Error {
			switch (runtime) {
				case DEFAULT:
					break;
				case QJS:
		...
		public Mumu.ScriptBackend? get_active_script_backend () {
		...
		private async void open (AgentSessionId id, HashTable<string, Variant> options,
			...
			try {
				sink = yield connection.get_proxy (null, ObjectPath.for_miru_message_sink (id), DO_NOT_LOAD_PROPERTIES,
					cancellable);
			} catch (IOError e) {
				throw_dbus_error (e);
			}
			...
			if (opts.realm == EMULATED) {
				...
				string sink_path = ObjectPath.for_miru_message_sink (id);
		...
		private async AgentSessionProvider get_emulated_provider (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				...
				emulated_worker = new Thread<void> ("miru-agent-emulated", run_emulated_agent);
		...
		private class NativeBridgeApi {
			...
			public static NativeBridgeApi open () throws Error {
				Mumu.Module? nb_mod = null;
				Mumu.Module? vm_mod = null;
				Mumu.Process.enumerate_modules (module => {
