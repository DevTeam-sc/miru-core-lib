namespace Miru {
	public sealed class Promise<T> {
		...
		private class Impl<T> : Object, Future<T> {
			...
			public async T wait_async (Cancellable? cancellable) throws Miru.Error, IOError {
				if (_ready)
					return get_result ();
			...
			public T get_result () throws Miru.Error, IOError {
				if (error != null) {
					if (error is Miru.Error)
						throw (Miru.Error) error;

					if (error is IOError.CANCELLED)
						throw (IOError) error;

					throw new Miru.Error.TRANSPORT ("%s", error.message);
				}
			...
			internal void abandon () {
				if (!_ready) {
					reject (new Miru.Error.INVALID_OPERATION ("Promise abandoned"));
				}
			}
	...
	public interface Future<T> : Object {
		...
		public abstract async T wait_async (Cancellable? cancellable) throws Miru.Error, IOError;
		public abstract void then (owned FutureCompletionHandler<T> handler);
		public abstract T get_result () throws Miru.Error, IOError;
	}
