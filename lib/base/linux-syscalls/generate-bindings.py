...
def render_c_source(
    ...
    if compat32_numbers is not None:
        _emit_c_signature_table(
            out,
            c_var="miru_compat32_syscall_signatures",
    ...
    _emit_c_signature_table_getter(
        out,
        c_func="miru_get_syscall_signatures",
        c_var="miru_syscall_signatures",
    )
    out.append("\n")
    _emit_c_signature_table_getter(
        out,
        c_func="miru_get_compat32_syscall_signatures",
        c_var="miru_compat32_syscall_signatures" if compat32_numbers is not None else None,
    )
...
def _emit_c_signature_table(
    ...
    *,
    ...
    out += [
        f"static const MiruLinuxSyscallSignature {c_var}[] =\n",
        "{\n",
    ]
    ...
    *,
    ...
    if c_var is not None:
        out += [
            ...
            f"  return (MiruLinuxSyscallSignature *) {c_var};\n",
        ]
    else:
        out += [
            "  if (len != NULL)\n",
