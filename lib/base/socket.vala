namespace Miru {
	...
	public async IOStream negotiate_connection (IOStream stream, WebServiceTransport transport, string host, string? origin,
		...
		msg.request_headers.replace ("User-Agent", "Miru/" + _version_string ());
		...
		var miru_context = MainContext.ref_thread_default ();
		...
		dbus_source.set_callback (() => {
			...
			var miru_source = new IdleSource ();
			miru_source.set_callback (negotiate_connection.callback);
			miru_source.attach (miru_context);
	...
	public sealed class WebService : Object {
		...
		private MainContext? miru_context;
		...
		private void schedule_on_miru_thread (owned SourceFunc function) {
		...
		private class ConnectionHandler : Object {
			...
			private void on_asset_request (Soup.Server server, Soup.ServerMessage msg, string path,
				...
				msg.get_response_headers ().replace ("Server", "Miru/" + _version_string ());
...
</html>""".printf ("Miru/" + _version_string ());
