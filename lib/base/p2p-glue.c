...
static gchar * miru_steal_bio_to_string (BIO ** bio);

static int miru_on_connection_output (void * addr, void * buffer, size_t length, uint8_t tos, uint8_t set_df);
static void miru_on_debug_printf (const char * format, ...);

static void miru_on_upcall (struct socket * sock, void * user_data, int flags);

void
_miru_generate_certificate (guint8 ** cert_der, gint * cert_der_length, gchar ** cert_pem, gchar ** key_pem)
{
  X509 * x509;
  X509_NAME * name;
  EVP_PKEY * pkey;
  ...
  X509_NAME_add_entry_by_txt (name, "O", MBSTRING_ASC, (const unsigned char *) "Miru", -1, -1, 0);
  ...
  *cert_pem = miru_steal_bio_to_string (&bio);

  bio = BIO_new (BIO_s_mem ());
  PEM_write_bio_PrivateKey (bio, pkey, NULL, NULL, 0, NULL, NULL);
  *key_pem = miru_steal_bio_to_string (&bio);
...
static gchar *
miru_steal_bio_to_string (BIO ** bio)
{
  gchar * result;
  long n;
  char * str;
...
void
_miru_sctp_connection_initialize_sctp_backend (void)
{
  const int msec_per_sec = 1000;

  usrsctp_init_nothreads (0, miru_on_connection_output, miru_on_debug_printf);
...
static int
miru_on_connection_output (void * addr, void * buffer, size_t length, uint8_t tos, uint8_t set_df)
{
  MiruSctpConnection * connection = addr;

  _miru_sctp_connection_emit_transport_packet (connection, buffer, (gint) length);

  return 0;
}

static void
miru_on_debug_printf (const char * format, ...)
{
  g_printerr ("[SCTP] %s\n", format);
}

void *
_miru_sctp_connection_create_sctp_socket (MiruSctpConnection * self)
{
  struct socket * sock;
  ...
  usrsctp_set_upcall (sock, miru_on_upcall, self);
...
void
_miru_sctp_connection_connect_sctp_socket (MiruSctpConnection * self, void * sock, guint16 port)
{
...
static void
miru_on_upcall (struct socket * sock, void * user_data, int flags)
{
  MiruSctpConnection * connection = user_data;

  _miru_sctp_connection_on_sctp_socket_events_changed (connection);
}

void
_miru_sctp_connection_close (void * sock)
{
  usrsctp_close (sock);
}

void
_miru_sctp_connection_shutdown (void * sock, MiruSctpShutdownType type, GError ** error)
{
...
GIOCondition
_miru_sctp_connection_query_sctp_socket_events (void * sock)
{
...
void
_miru_sctp_connection_handle_transport_packet (MiruSctpConnection * self, guint8 * data, gint data_length)
{
  usrsctp_conninput (self, data, data_length, 0);
}

gssize
_miru_sctp_connection_recv (void * sock, guint8 * buffer, gint buffer_length, guint16 * stream_id, MiruPayloadProtocolId * protocol_id,
    MiruSctpMessageFlags * message_flags, GError ** error)
{
  gssize n;
  ...
  {
    *stream_id = 0;
    *protocol_id = MIRU_PAYLOAD_PROTOCOL_ID_NONE;
  }

  *message_flags = 0;

  if ((msg_flags & MSG_EOR) != 0)
    *message_flags |= MIRU_SCTP_MESSAGE_FLAGS_END_OF_RECORD;

  if ((msg_flags & MSG_NOTIFICATION) != 0)
    *message_flags |= MIRU_SCTP_MESSAGE_FLAGS_NOTIFICATION;
...
gssize
_miru_sctp_connection_send (void * sock, guint16 stream_id, MiruPayloadProtocolId protocol_id, guint8 * data, gint data_length,
      GError ** error)
{
  gssize n;
...
gint
_miru_sctp_timer_source_get_timeout (void)
{
  return usrsctp_get_timeout ();
}

void
_miru_sctp_timer_source_process_timers (guint32 elapsed_msec)
{
