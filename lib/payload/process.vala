namespace Miru {
	...
	public Mumu.MemoryRange detect_own_range_and_path (Mumu.MemoryRange? mapped_range, out string? path) {
		Mumu.MemoryRange? own_range = mapped_range;
		string? own_path = null;

		if (own_range == null) {
			Mumu.Address our_address = Mumu.Address.from_pointer (Mumu.strip_code_pointer ((void *) detect_own_range_and_path));
			...
			if (own_range == null) {
				Mumu.Process.enumerate_ranges (READ | EXECUTE, details => {
					var range = details.range;
	...
	public interface ProcessInvader : Object {
		...
		public abstract Mumu.MemoryRange get_memory_range ();

		public abstract Mumu.ScriptBackend get_script_backend (ScriptRuntime runtime) throws Error;
		public abstract Mumu.ScriptBackend? get_active_script_backend ();
