...
#else
	public sealed class ForkMonitor : Object, Mumu.InvocationListener {
		public weak ForkHandler handler {
			get;
			construct;
		}
		...
		construct {
			...
			unowned Mumu.InvocationListener listener = this;
			...
			if (get_executable_path ().has_prefix ("/system/bin/app_process")) {
				try {
					...
					if (cmdline == "zygote" || cmdline == "zygote64" || cmdline == "usap32" || cmdline == "usap64") {
						var runtime = Mumu.Process.find_module_by_name ("libandroid_runtime.so");
						if (runtime != null) {
						...
						var selinux = Mumu.Process.find_module_by_name ("libselinux.so");
						if (selinux != null) {
		...
		public void on_fork_enter (Mumu.InvocationContext context) {
			state = FORKING;
			identifier = null;
			handler.prepare_to_fork ();
		}

		public void on_fork_leave (Mumu.InvocationContext context) {
		...
		public void on_set_argv0_enter (Mumu.InvocationContext context) {
			if (identifier == null) {
		...
		public void on_set_argv0_leave (Mumu.InvocationContext context) {
			if (state == FORKING) {
		...
		public void on_set_ctx_enter (Mumu.InvocationContext context) {
		...
		public void on_set_ctx_leave (Mumu.InvocationContext context) {
			if (state == IDLE)
