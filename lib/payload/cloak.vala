namespace Miru {
	public sealed class ThreadIgnoreScope {
		...
		private Mumu.ThreadId thread_id;

		private uint num_ranges;
		private Mumu.MemoryRange ranges[2];
		...
		public ThreadIgnoreScope (Kind kind) {
			...
			if (kind == MIRU_THREAD) {
				thread_id = Mumu.Process.get_current_thread_id ();
				Mumu.Cloak.add_thread (thread_id);

				num_ranges = Mumu.Thread.try_get_ranges (ranges);
				for (var i = 0; i != num_ranges; i++)
					Mumu.Cloak.add_range (ranges[i]);
			}
		}
		...
		~ThreadIgnoreScope () {
			if (kind == MIRU_THREAD) {
				for (var i = 0; i != num_ranges; i++)
					Mumu.Cloak.remove_range (ranges[i]);

				Mumu.Cloak.remove_thread (thread_id);
			}
	...
	public sealed class ThreadCountCloaker : Object {
		...
		construct {
			var art = Mumu.Process.find_module_by_name ("libart.so");
			if (art != null) {
		...
		private static void * update_slot (void ** orig, void * new_impl) {
			Mumu.PageProtection old_prot = READ;
			Mumu.Memory.query_protection (orig, out old_prot);

			bool is_writable = (old_prot & Mumu.PageProtection.WRITE) != 0;
			if (!is_writable)
				Mumu.mprotect (orig, sizeof (void *), old_prot | WRITE);

			void * old_impl = *orig;
			*orig = new_impl;

			if (!is_writable)
				Mumu.mprotect (orig, sizeof (void *), old_prot);
	...
	public sealed class ThreadListCloaker : Object, DirListFilter {
		...
		private bool matches_file (string name) {
			var tid = (Mumu.ThreadId) uint64.parse (name);
			return Mumu.Cloak.has_thread (tid);
		}
	}
	...
	public sealed class FDListCloaker : Object, DirListFilter {
		...
		private bool matches_file (string name) {
			...
			return Mumu.Cloak.has_file_descriptor (fd);
		}
	}
	...
	private sealed class DirListCloaker : Object {
		...
		private Gee.HashSet<Mumu.InvocationListener> listeners = new Gee.HashSet<Mumu.InvocationListener> ();
		...
		private class OpenDirListener : Object, Mumu.InvocationListener {
			...
			public void on_enter (Mumu.InvocationContext context) {
			...
			public void on_leave (Mumu.InvocationContext context) {
				Invocation * invocation = context.get_listener_invocation_data (sizeof (Invocation));
		...
		private class CloseDirListener : Object, Mumu.InvocationListener {
			public weak DirListCloaker parent {
				get;
				construct;
			}
		...
		private class ReadDirListener : Object, Mumu.InvocationListener {
			public weak DirListCloaker parent {
				get;
				construct;
			}
		...
		private class ReadDirRListener : Object, Mumu.InvocationListener {
			public weak DirListCloaker parent {
				get;
				construct;
			}
