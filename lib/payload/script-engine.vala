namespace Miru {
	public sealed class ScriptEngine : Object {
		...
		public async ScriptInstance create_script (string? source, Bytes? bytes, ScriptOptions options) throws Error {
			...
			Mumu.ScriptBackend backend = pick_backend (options.runtime);
		...
		private Mumu.ScriptBackend pick_backend (ScriptRuntime runtime) throws Error {
			if (runtime == DEFAULT)
		...
		public Mumu.Script eternalize_script (AgentScriptId script_id) throws Error {
		...
		private static void schedule_on_js_thread (owned SourceFunc function) {
			var source = new IdleSource ();
			source.set_callback ((owned) function);
			source.attach (Mumu.ScriptBackend.get_scheduler ().get_js_context ());
		}
		...
		public sealed class ScriptInstance : Object, RpcPeer {
			...
			public ScriptInstance (AgentScriptId script_id, Mumu.Script script) {
			...
			public async void close () {
				...
				if (state == DISPOSED) {
					...
					js_source.attach (Mumu.ScriptBackend.get_scheduler ().get_js_context ());
					yield;
			...
			public Mumu.Script eternalize () throws Error {
