...
void
_miru_thread_suspend_monitor_remove_cloaked_threads (task_inspect_t task, thread_act_array_t * threads, mach_msg_type_number_t * count)
{
  guint i, o;
  ...
  for (i = 0, o = 0; i != *count; i++)
  {
    ...
    if (mumu_cloak_has_thread (thread))
  ...
  pages_before = MUMU_ALIGN_SIZE (old_size, page_size) / page_size;
  pages_after = MUMU_ALIGN_SIZE (new_size, page_size) / page_size;
