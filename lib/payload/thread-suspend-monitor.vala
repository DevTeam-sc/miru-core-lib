namespace Miru {
	...
	public sealed class ThreadSuspendMonitor : Object {
		...
		construct {
			...
			var kernel = Mumu.Process.find_module_by_name (LIBSYSTEM_KERNEL);
		...
		private static int replacement_task_threads (uint task_id, uint ** threads, uint * count) {
			...
			if (monitor.is_called_by_miru (context))
		...
		private int handle_thread_suspend (uint thread_id) {
			if (Mumu.Cloak.has_thread (thread_id))
				return 0;

			var script_backend = invader.get_active_script_backend ();
			uint caller_thread_id = (uint) Mumu.Process.get_current_thread_id ();
			...
			while (true) {
				script_backend.with_lock_held (() => {
					interceptor.with_lock_held (() => {
						Mumu.Cloak.with_lock_held (() => {
							result = thread_suspend (thread_id);
						});
					});
				});

				if (result != 0 || (!script_backend.is_locked () && !Mumu.Cloak.is_locked () && !interceptor.is_locked ()))
					break;
		...
		private bool is_called_by_miru (Mumu.InvocationContext context) {
			Mumu.MemoryRange range = invader.get_memory_range ();
			var caller = Mumu.Address.from_pointer (context.get_return_address ());
