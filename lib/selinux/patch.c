...
#define MIRU_SELINUX_ERROR miru_selinux_error_quark ()

typedef struct _MiruSELinuxRule MiruSELinuxRule;
typedef enum _MiruSELinuxErrorEnum MiruSELinuxErrorEnum;

struct _MiruSELinuxRule
{
...
enum _MiruSELinuxErrorEnum
{
  MIRU_SELINUX_ERROR_POLICY_FORMAT_NOT_SUPPORTED,
  MIRU_SELINUX_ERROR_TYPE_NOT_FOUND,
  MIRU_SELINUX_ERROR_CLASS_NOT_FOUND,
  MIRU_SELINUX_ERROR_PERMISSION_NOT_FOUND
};

static gboolean miru_load_policy (const gchar * filename, policydb_t * db, gchar ** data, GError ** error);
static gboolean miru_save_policy (const gchar * filename, policydb_t * db, GError ** error);
static type_datum_t * miru_ensure_type (policydb_t * db, const gchar * type_name, guint num_attributes, ...);
static void miru_add_type_to_class_constraints_referencing_attribute (policydb_t * db, uint32_t type_id, uint32_t attribute_id);
static gboolean miru_ensure_permissive (policydb_t * db, const gchar * type_name, GError ** error);
static avtab_datum_t * miru_ensure_rule (policydb_t * db, const gchar * s, const gchar * t, const gchar * c, const gchar * p, GError ** error);

static gboolean miru_set_file_contents (const gchar * filename, const gchar * contents, gssize length, GError ** error);

static const MiruSELinuxRule miru_selinux_rules[] =
{
  { { "domain", NULL }, "domain", "process", { "execmem", NULL } },
  { { "domain", NULL }, "miru_file", "dir", { "search", NULL } },
  { { "domain", NULL }, "miru_file", "file", { "open", "read", "getattr", "execute", "?map", NULL } },
  { { "domain", NULL }, "miru_memfd", "file", { "open", "read", "write", "getattr", "execute", "?map", NULL } },
...
G_DEFINE_QUARK (miru-selinux-error-quark, miru_selinux_error)

void
miru_selinux_patch_policy (void)
{
  ...
  if (!miru_load_policy (system_policy, &db, &db_data, &error))
  {
  ...
  if (miru_ensure_type (&db, "miru_file", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
  ...
  if (miru_ensure_type (&db, "miru_memfd", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
    g_printerr ("Unable to add SELinux type: %s\n", error->message);
    g_clear_error (&error);
    goto beach;
  ...
  for (rule_index = 0; rule_index != G_N_ELEMENTS (miru_selinux_rules); rule_index++)
  {
    const MiruSELinuxRule * rule = &miru_selinux_rules[rule_index];
    ...
    for (source_cursor = rule->sources; *source_cursor != NULL; source_cursor++)
    {
      ...
      for (perm_entry = rule->permissions; *perm_entry != NULL; perm_entry++)
      {
        ...
        if (miru_ensure_rule (&db, source, target, rule->klass, perm, &error) == NULL)
        {
          if (!g_error_matches (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_PERMISSION_NOT_FOUND) || is_important)
  ...
  if (!miru_save_policy ("/sys/fs/selinux/load", &db, &error))
  {
    ...
    if (probably_in_emulator)
    {
      ...
      success = miru_ensure_permissive (&db, "shell", &error);
      if (success)
        success = miru_save_policy ("/sys/fs/selinux/load", &db, &error);
...
static gboolean
miru_load_policy (const gchar * filename, policydb_t * db, gchar ** data, GError ** error)
{
  policy_file_t file;
  int res;
  ...
  *data = file.data;
  ...
  if (res != 0)
  {
    g_set_error (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_POLICY_FORMAT_NOT_SUPPORTED, "unsupported policy database format");
    policydb_destroy (db);
    g_free (*data);
    return FALSE;
  }
...
static gboolean
miru_save_policy (const gchar * filename, policydb_t * db, GError ** error)
{
  void * data;
  size_t size;
  int res G_GNUC_UNUSED;
  ...
  return miru_set_file_contents (filename, data, size, error);
}

static type_datum_t *
miru_ensure_type (policydb_t * db, const gchar * type_name, guint n_attributes, ...)
{
  type_datum_t * type;
  uint32_t type_id;
  va_list vl;
  ...
  for (i = 0; i != n_attributes; i++)
  {
    ...
    if (attribute_type != NULL)
    {
      ...
      miru_add_type_to_class_constraints_referencing_attribute (db, type_id, attribute_id);
    }
    else if (pending_error == NULL)
    {
      g_set_error (&pending_error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_TYPE_NOT_FOUND, "attribute type “%s” does not exist", attribute_name);
    }
  }
...
static void
miru_add_type_to_class_constraints_referencing_attribute (policydb_t * db, uint32_t type_id, uint32_t attribute_id)
{
...
static gboolean
miru_ensure_permissive (policydb_t * db, const gchar * type_name, GError ** error)
{
  type_datum_t * type;
  int res G_GNUC_UNUSED;
  ...
  if (type == NULL)
  {
    g_set_error (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_TYPE_NOT_FOUND, "type %s does not exist", type_name);
    return FALSE;
  }
...
miru_ensure_rule (policydb_t * db, const gchar * s, const gchar * t, const gchar * c, const gchar * p, GError ** error)
{
  ...
  if (source == NULL)
  {
    g_set_error (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_TYPE_NOT_FOUND, "source type “%s” does not exist", s);
    return NULL;
  }
  ...
  if (target == NULL)
  {
    g_set_error (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_TYPE_NOT_FOUND, "target type “%s” does not exist", t);
    return NULL;
  }
  ...
  if (klass == NULL)
  {
    g_set_error (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_CLASS_NOT_FOUND, "class “%s” does not exist", c);
    return NULL;
  }
  ...
  if (perm == NULL)
  {
    g_set_error (error, MIRU_SELINUX_ERROR, MIRU_SELINUX_ERROR_PERMISSION_NOT_FOUND, "perm “%s” does not exist on the “%s” class", p, c);
    return NULL;
  }
...
static gboolean
miru_set_file_contents (const gchar * filename, const gchar * contents, gssize length, GError ** error)
{
  int fd, res;
  gsize offset, size;
