...
typedef struct _MiruInitMessage MiruInitMessage;

struct _MiruInitMessage
{
...
_miru_pipe_transport_create_backend (gchar ** local_address, gchar ** remote_address, GError ** error)
{
  ...
  for (i = 0; i != G_N_ELEMENTS (sockets); i++)
  {
    ...
    miru_unix_socket_tune_buffer_sizes (fd);
  }
...
gint
_miru_darwin_pipe_consume_stashed_file_descriptor (const gchar * address, GError ** error)
{
  gint fd = -1;
  G_GNUC_UNUSED gint assigned;
  mach_port_t port = MACH_PORT_NULL;
  MiruInitMessage init = { { 0, }, { 0, } };
  kern_return_t kr;
  const gchar * failed_operation;
  mach_port_t wrapper;
