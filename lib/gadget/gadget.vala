namespace Miru.Gadget {
	private sealed class Config : Object, Json.Serializable {
		...
		public Mumu.TeardownRequirement teardown {
			get;
			set;
			default = Mumu.TeardownRequirement.MINIMAL;
		}
		...
		public Mumu.CodeSigningPolicy code_signing {
			get;
			set;
			default = Mumu.CodeSigningPolicy.OPTIONAL;
		}
	...
	private sealed class Location : Object {
		...
		public Location (string executable_name, string? path, Mumu.MemoryRange range) {
			Object (
	...
	private Mumu.Interceptor? interceptor;
	private Mumu.Exceptor? exceptor;
	private Mutex mutex;
	private Cond cond;

	public void load (Mumu.MemoryRange? mapped_range, string? config_data, int * result) {
		if (loaded)
			return;
		loaded = true;
		...
		Mumu.Process.set_teardown_requirement (config.teardown);
		Mumu.Process.set_code_signing_policy (config.code_signing);

		Mumu.Cloak.add_range (location.range);

		interceptor = Mumu.Interceptor.obtain ();
		...
		if (wait_for_resume_needed && Environment.can_block_at_load_time ()) {
			var scheduler = Mumu.ScriptBackend.get_scheduler ();
	...
	public void unload () {
		...
		if (config.teardown == Mumu.TeardownRequirement.FULL) {
			config = null;
	...
	private async void perform_start (Gee.Promise<int>? request) {
		worker_ignore_scope = new ThreadIgnoreScope (MIRU_THREAD);
		...
		try {
			...
			if (server != null) {
				...
				if (inet_address != null) {
					...
					Environment.set_thread_name ("miru-gadget-tcp-%u".printf (listen_port));
					if (request != null) {
					...
					Environment.set_thread_name ("miru-gadget-unix");
					if (request != null) {
						request.set_value (0);
					} else {
	...
	private async void stop () {
		...
		if (controller != null) {
			if (controller.is_eternal) {
				...
				if (config.teardown == Mumu.TeardownRequirement.MINIMAL) {
	...
	private Location detect_location (Mumu.MemoryRange? mapped_range) {
		string? executable_name = null;
		string? our_path = null;
		Mumu.MemoryRange? our_range = mapped_range;

		Mumu.Address our_address = Mumu.Address.from_pointer (Mumu.strip_code_pointer ((void *) detect_location));

#if DARWIN
		Environment.detect_darwin_location_fields (our_address, ref executable_name, ref our_path, ref our_range);
#else
		var index = 0;
		Mumu.Process.enumerate_modules ((details) => {
	...
	private abstract class BaseController : Object, Controller, ProcessInvader, ExitHandler {
		...
		private Mumu.ScriptBackend? qjs_backend;
		private Mumu.ScriptBackend? v8_backend;
		...
		protected Mumu.MemoryRange get_memory_range () {
			return location.range;
		}

		protected Mumu.ScriptBackend get_script_backend (ScriptRuntime runtime) throws Error {
			switch (runtime) {
				case DEFAULT:
				...
				case QJS:
					if (qjs_backend == null) {
						qjs_backend = Mumu.ScriptBackend.obtain_qjs ();
						if (qjs_backend == null) {
				...
				case V8:
					if (v8_backend == null) {
						v8_backend = Mumu.ScriptBackend.obtain_v8 ();
						if (v8_backend == null) {
							throw new Error.NOT_SUPPORTED (
		...
		protected Mumu.ScriptBackend? get_active_script_backend () {
		...
		protected async PortalMembershipId join_portal (string address, PortalOptions options,
			...
			client.resume.connect (Miru.Gadget.resume);
			client.kill.connect (Miru.Gadget.kill);
		...
		private bool supports_async_exit () {
			...
			return Mumu.Process.has_thread (Environment.get_worker_tid ());
		}
	...
	private sealed class ControlServer : BaseController {
		...
		private Gee.ArrayList<Mumu.Script> eternalized_scripts = new Gee.ArrayList<Mumu.Script> ();
		...
		private async AgentSessionId attach (HashTable<string, Variant> options, ControlChannel requester,
			...
			MiruMessageSink sink;
			try {
				sink = yield controller_connection.get_proxy (null, ObjectPath.for_miru_message_sink (id),
		...
		private async void reattach (AgentSessionId id, ControlChannel requester, Cancellable? cancellable) throws Error, IOError {
			...
			try {
				session.message_sink = yield controller_connection.get_proxy (null, ObjectPath.for_miru_message_sink (id),
					DO_NOT_LOAD_PROPERTIES, cancellable);
			} catch (IOError e) {
				throw_dbus_error (e);
			}
		...
		private void on_script_eternalized (Mumu.Script script) {
		...
		private class ControlChannel : Object, Peer, HostSession, GadgetSession {
			...
			construct {
				try {
					...
					registrations.add (connection.register_object (Miru.ObjectPath.HOST_SESSION, host_session));

					GadgetSession gadget_session = this;
					registrations.add (connection.register_object (Miru.ObjectPath.GADGET_SESSION, gadget_session));

					AuthenticationService null_auth = new NullAuthenticationService ();
					registrations.add (connection.register_object (Miru.ObjectPath.AUTHENTICATION_SERVICE, null_auth));
				} catch (IOError e) {
					assert_not_reached ();
				}

				uint pid = get_process_id ();
				string identifier = "re.miru.Gadget";
				string name = "Gadget";
		...
		private class LiveAgentSession : BaseAgentSession {
			...
			public LiveAgentSession (ProcessInvader invader, AgentSessionId id, uint persist_timeout, MiruMessageSink sink,
				...
				Object (
					...
					message_sink: sink,
					miru_context: MainContext.ref_thread_default (),
	...
	namespace Environment {
		...
		private extern Mumu.ThreadId get_worker_tid ();
...
#if DARWIN
		private extern void detect_darwin_location_fields (Mumu.Address our_address, ref string? executable_name,
			ref string? our_path, ref Mumu.MemoryRange? our_range);
