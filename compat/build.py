...
if not shutil.which(NINJA):
    potential_ninja = REPO_ROOT.parent.parent / "deps" / "toolchain-windows-x86" / "bin" / "ninja.exe"
    if potential_ninja.exists():
        NINJA = str(potential_ninja)

print(f"DEBUG: NINJA path resolved to: {NINJA}", file=sys.stderr)
...
def main(argv):
    ...
    command.add_argument("miru_version", help="the Miru version")
    ...
    try:
        ...
        if "native" in compat:
            ...
            if host_os == "windows" and host_arch == "x86_64" and have_toolchain:
                ...
                if "gadget" in components and False:
                    outputs[group] += [
            ...
            if host_os in {"macos", "ios"} and host_arch in {"arm64e", "arm64"} and host_config != "simulator":
                ...
                outputs[group] = [
                    Output(identifier=f"helper_{kind}",
                           name=f"miru-helper-{other_arch}",
                           file=HELPER_FILE_UNIX,
                           target=HELPER_TARGET),
                    Output(identifier=f"agent_{kind}",
                           name=f"miru-agent-{other_arch}.dylib",
                ...
                if "gadget" in components:
                    outputs[group] += [
                        Output(identifier=f"gadget_{kind}",
                               name=f"miru-gadget-{other_arch}.dylib",
                ...
                if host_arch == "x86_64":
                    outputs[OutputGroup("arm64")] = [
                        Output(identifier="agent_emulated_modern",
                               name="miru-agent-arm64.so",
                               file=AGENT_FILE_ELF,
                               target=AGENT_TARGET),
                    ]

        raw_allowed_prebuilds = os.environ.get("MIRU_ALLOWED_PREBUILDS")
        allowed_prebuilds = set(raw_allowed_prebuilds.split(",")) if raw_allowed_prebuilds is not None else None

        state = State(role, builddir, top_builddir, miru_version, host_os, host_config, allowed_prebuilds, outputs)
...
def compile(privdir: Path, state: State):
    ...
    def call_internal_meson(argv, *args, **kwargs):
        # DEBUG: Don't silence output to debug failures
        ...
        return call_meson(argv, *args, **kwargs)

    options: Optional[Sequence[str]] = None
    build_env = scrub_environment(os.environ)
    if 'ANDROID_NDK_ROOT' not in build_env:
        build_env['ANDROID_NDK_ROOT'] = r"l:\tango\MIRU_Magisk_KTB_Workspace\miruLibrary\deps\android-ndk-r25c"
        print(f"DEBUG: Hardcoded ANDROID_NDK_ROOT injected: {build_env['ANDROID_NDK_ROOT']}")
    build_env["MIRU_RELENG"] = str(releng_location)
    ...
    for group, outputs in state.outputs.items():
        ...
        if not (workdir / "build.ninja").exists():
            if options is None:
                ...
                version_opt = next((opt for opt in options if opt.startswith("-Dmiru_version=")), None)
                if version_opt is None:
                    options += [f"-Dmiru_version={state.miru_version}"]
...
def adapt_key(k: "OptionKey", role: Role) -> "OptionKey":
    if role == "subproject" and k.subproject == "miru-core":
        return k.as_root()
    return k
...
def option_should_be_forwarded(k: "OptionKey",
    ...
    our_project_id = "miru-core" if role == "subproject" else ""
...
def scrub_environment(env: Mapping[str, str]) -> Mapping[str, str]:
    print(f"DEBUG: scrub_environment called. ANDROID_NDK_ROOT present: {'ANDROID_NDK_ROOT' in env}")
    if 'ANDROID_NDK_ROOT' in env:
        print(f"DEBUG: ANDROID_NDK_ROOT value: {env['ANDROID_NDK_ROOT']}")
    from releng.env import TOOLCHAIN_ENVVARS
    clean_env = OrderedDict()
    envvars_to_avoid = {*TOOLCHAIN_ENVVARS, *MSVS_ENVVARS}
    # DEBUG: Disable scrubbing for MSVS to fix build machine compiler detection
    ...
    # Inject MSVC paths if missing (for Windows build machine)
    if platform.system() == "Windows":
        msvc_include = [
            r"C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC\14.44.35207\include",
        ...
        msvc_lib = [
            r"C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC\14.44.35207\lib\x64",
        ...
        def append_path(env_key, paths):
            current = clean_env.get(env_key, "")
            new_paths = ";".join(paths)
            if current:
                clean_env[env_key] = current + ";" + new_paths
            else:
                clean_env[env_key] = new_paths
            print(f"DEBUG: Injected {env_key}: {clean_env[env_key]}")

        append_path("INCLUDE", msvc_include)
        append_path("LIB", msvc_lib)
...
def detect_relevant_subprojects(releng_location: Path) -> dict[str, Path]:
    ...
    mumu_location = subprojects.get("miru-mumu")
    if mumu_location is not None:
        subprojects.update(detect_relevant_subprojects_in(mumu_location, releng_location))
    return subprojects
...
HELPER_TARGET = "miru-helper"
HELPER_FILE_WINDOWS = Path("src") / "miru-helper.exe"
HELPER_FILE_UNIX = Path("src") / "miru-helper"

AGENT_TARGET = "miru-agent"
AGENT_FILE_WINDOWS = Path("lib") / "agent" / "miru-agent.dll"
AGENT_FILE_DARWIN = Path("lib") / "agent" / "miru-agent.dylib"
AGENT_FILE_ELF = Path("lib") / "agent" / "miru-agent.so"

GADGET_TARGET = "miru-gadget"
GADGET_FILE_WINDOWS = Path("lib") / "gadget" / "miru-gadget.dll"
GADGET_FILE_DARWIN = Path("lib") / "gadget" / "miru-gadget.dylib"
GADGET_FILE_ELF = Path("lib") / "gadget" / "miru-gadget.so"

SERVER_TARGET = "miru-server"
SERVER_FILE_UNIX = Path("server") / "miru-server"
