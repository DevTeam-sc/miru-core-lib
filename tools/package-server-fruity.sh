...
if [ -z "$MIRU_VERSION" ]; then
  echo "MIRU_VERSION must be set" > /dev/stderr
  exit 2
fi
...
executable=$prefix/usr/bin/miru-server
if [ ! -f "$executable" ]; then
  echo "$executable: not found" > /dev/stderr
  exit 4
fi

agent=$prefix/usr/lib/miru/miru-agent.dylib
if [ ! -f "$agent" ]; then
...
libdir=$pkroot/usr/lib/miru
daedir=$pkroot/Library/LaunchDaemons

mkdir -p "$bindir/"
cp "$executable" "$bindir/miru-server"
chmod 755 "$bindir/miru-server"

mkdir -p "$libdir/"
cp "$agent" "$libdir/miru-agent.dylib"
chmod 755 "$libdir/miru-agent.dylib"
...
(
  ...
  echo "<dict>"
  echo "	<key>Label</key>"
  echo "	<string>re.miru.server</string>"
  echo "	<key>Program</key>"
  echo "	<string>$sysroot/usr/sbin/miru-server</string>"
  ...
  echo "</dict>"
  echo "</plist>"
) > "$daedir/re.miru.server.plist"
chmod 644 "$daedir/re.miru.server.plist"

installed_size=$(du -sk "$tmpdir" | cut -f1)

mkdir -p "$tmpdir/DEBIAN/"
cat >"$tmpdir/DEBIAN/control" <<EOF
Package: re.miru.server
Name: Miru
...
Author: Miru Developers <oleavr@nowsecure.com>
Section: Development
Conflicts: re.miru.server64
EOF
...
launchcfg=$sysroot/Library/LaunchDaemons/re.miru.server.plist
launchlog=\$(mktemp)
...
if [ "\$1" = remove ] || [ "\$1" = purge ]; then
  launchctl unload $sysroot/Library/LaunchDaemons/re.miru.server.plist &> /dev/null
fi
