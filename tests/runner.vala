...
namespace Miru.Test {
	public static void run (string[] args) {
			...
			try {
				switch (os ()) {
					case MACOS:
						switch (cpu ()) {
							case ARM_64:
								if (Mumu.query_ptrauth_support () == UNSUPPORTED) {
									string output;
		...
		Miru.SystemTest.add_tests ();

#if HAVE_LOCAL_BACKEND
		Miru.InjectorTest.add_tests ();

		Miru.AgentTest.add_tests ();
#endif
#if HAVE_GADGET && !WINDOWS
		Miru.GadgetTest.add_tests ();
#endif
		Miru.HostSessionTest.add_tests ();

#if HAVE_COMPILER_BACKEND && !QNX
		Miru.CompilerTest.add_tests ();
#endif
	...
	public static string path_to_temporary_file (string name) {
		var prefix = "miru-tests-%u-".printf (Mumu.Process.get_id ());
#if QNX
