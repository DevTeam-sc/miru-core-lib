...
export MIRU_TEST_LOG=$test_log
export MIRU_V8_EXTRA_FLAGS="--logfile=$v8_log --no-logfile-per-isolate --log-code --interpreted-frames-native-stack"

sudo --preserve-env=MIRU_TEST_LOG,MIRU_V8_EXTRA_FLAGS dtrace \
  -c "$builddir/tests/miru-tests -p /Compiler/Performance/build-simple-agent" \
  -x ustackframes=100 \
...
"$repo/tools/symbolicate.py" \
  ...
  --test-log "$test_log" \
  --v8-log "$v8_log" \
  --agent "$builddir/lib/agent/libmiru-agent-modulated.dylib"
"$flamegraph/stackcollapse.pl" "$stacks_symbolicated" \
  | grep mumu_script_scheduler_run_js_loop \
  > "$stacks_folded"
