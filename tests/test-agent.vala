namespace Miru.AgentTest {
	...
	namespace Script {
		private static async void load_and_receive_messages (Harness h) {
			...
			func (1337, "Miru rocks");

			var message = yield h.wait_for_message ();
			assert_true (message.script_id.handle == script_id.handle);
			assert_true (message.text == "{\"type\":\"send\",\"payload\":{\"first_argument\":1337,\"second_argument\":\"Miru rocks\"}}");
...
""", make_parameters_dict (), cancellable);
				...
				for (uint i = 0; i != 1000000; i++) {
					...
					while (true) {
						...
						assert_true (reader.get_string_value () == "miru:rpc");
						reader.end_element ();
					...
					var child = Miru.Test.Process.start ("/bin/ls", new string[] {
...
""", make_parameters_dict (), cancellable);
	...
	private sealed class Harness : Miru.Test.AsyncHarness, AgentController, MiruMessageSink {
		private GLib.Module module;
		[CCode (has_target = false)]
		private delegate void AgentMainFunc (string data, ref Miru.UnloadPolicy unload_policy, void * opaque_injector_state);
		...
		public async AgentSession load_agent () {
			...
			var deployment_dir = Path.get_dirname (Miru.Test.Process.current.filename);
			agent_filename = Path.build_filename (deployment_dir, "miru-agent." + shlib_extension);
#else
			var miru_root_dir = Path.get_dirname (Path.get_dirname (Miru.Test.Process.current.filename));
			agent_filename = Path.build_filename (miru_root_dir, "lib", "miru", "miru-agent." + shlib_extension);
			if (!FileUtils.test (agent_filename, FileTest.EXISTS))
				agent_filename = Path.build_filename (miru_root_dir, "lib", "agent", "miru-agent." + shlib_extension);
#endif
			...
			var main_func_found = module.symbol ("miru_agent_main", out main_func_symbol);
			...
			main_thread = new Thread<bool> ("miru-test-agent-worker", agent_main_worker);
			...
			try {
				...
				registrations.add_all_array ({
					...
					connection.register_object (ObjectPath.for_miru_message_sink (session_id), (MiruMessageSink) this)
				});
