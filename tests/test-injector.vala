namespace Miru.InjectorTest {
	public static void add_tests () {
		...
		GLib.Test.add_func ("/Injector/suspended-injection-current-arch", () => {
			test_suspended_injection (Miru.Test.Arch.CURRENT);
		});

		if (can_test_cross_arch_injection) {
			GLib.Test.add_func ("/Injector/suspended-injection-other-arch", () => {
				test_suspended_injection (Miru.Test.Arch.OTHER);
			});
		}
#endif
	}

	private static void test_dynamic_injection (Miru.Test.Arch arch) {
		var logfile = File.new_for_path (Miru.Test.path_to_temporary_file ("dynamic-injection.log"));
		try {
			logfile.delete ();
		} catch (GLib.Error delete_error) {
		}
		var envp = new string[] {
			"MIRU_LABRAT_LOGFILE=" + logfile.get_path ()
		};
		...
		switch (Miru.Test.os ()) {
			case Miru.Test.OS.MACOS:   // Mumu.Darwin.Mapper
			case Miru.Test.OS.IOS:     // Mumu.Darwin.Mapper
			case Miru.Test.OS.TVOS:    // Mumu.Darwin.Mapper
			case Miru.Test.OS.ANDROID: // Bionic's behavior
				assert_true (content_of (logfile) == ">m<>m");
				break;
			case Miru.Test.OS.LINUX:
				if (Miru.Test.libc () == Miru.Test.Libc.UCLIBC) {
					assert_true (content_of (logfile) == ">m<>m");
				} else {
					assert_true (content_of (logfile) == ">m<>m<");
				}
	...
	private static void test_resident_injection (Miru.Test.Arch arch) {
		var logfile = File.new_for_path (Miru.Test.path_to_temporary_file ("resident-injection.log"));
		try {
			logfile.delete ();
		} catch (GLib.Error delete_error) {
		}
	...
	private static void test_resource_leaks () {
		var logfile = File.new_for_path (Miru.Test.path_to_temporary_file ("leaks.log"));
		var envp = new string[] {
			"MIRU_LABRAT_LOGFILE=" + logfile.get_path ()
		};
...
#if DARWIN
	private static void test_suspended_injection (Miru.Test.Arch arch) {
		var logfile = File.new_for_path (Miru.Test.path_to_temporary_file ("suspended-injection.log"));
		try {
			logfile.delete ();
		} catch (GLib.Error delete_error) {
		}
	...
	private sealed class Labrat {
		public Miru.Test.Process? process {
			get;
			private set;
		}
		...
		public Labrat (string name, string[] envp, Miru.Test.Arch arch = Miru.Test.Arch.CURRENT) {
			try {
				process = Miru.Test.Process.start (Miru.Test.Labrats.path_to_executable (name), null, envp, arch);
			} catch (Error e) {
				printerr ("\nFAIL: %s\n\n", e.message);
				assert_not_reached ();
			}
		...
		public Labrat.suspended (string name, string[] envp, Miru.Test.Arch arch = Miru.Test.Arch.CURRENT) {
			try {
				process = Miru.Test.Process.create (Miru.Test.Labrats.path_to_executable (name), null, envp, arch);
			} catch (Error e) {
				printerr ("\nFAIL: %s\n\n", e.message);
				assert_not_reached ();
			}
		...
		public void inject (string name, string data, Miru.Test.Arch arch = Miru.Test.Arch.CURRENT) {
			var loop = new MainLoop ();
			Idle.add (() => {
		...
		private async void perform_injection (string name, string data, Miru.Test.Arch arch, MainLoop loop) {
			if (injector == null) {
			...
			try {
				var path = Miru.Test.Labrats.path_to_library (name, arch);
				assert_true (FileUtils.test (path, FileTest.EXISTS));

				yield injector.inject_library_file (process.id, path, "miru_agent_main", data);
			} catch (GLib.Error e) {
				printerr ("\nFAIL: %s\n\n", e.message);
				assert_not_reached ();
			}
