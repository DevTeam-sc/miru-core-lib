		...
		private static void load_script () {
			...
			switch (Miru.Test.os ()) {
				case Miru.Test.OS.MACOS: {
					var tests_dir = Path.get_dirname (Miru.Test.Process.current.filename);
			...
			var gadget_filename = Path.build_filename (gadget_dir, "miru-gadget" + Miru.Test.os_library_suffix ());
			var config_filename = Path.build_filename (gadget_dir, "miru-gadget.config");
			...
			try {
				...
				var process = Miru.Test.Process.start (Miru.Test.Labrats.path_to_executable ("sleeper"), null, envp);
