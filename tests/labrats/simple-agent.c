...
void
miru_agent_main (const char * data)
{
  append_to_log ('m');
  ...
  else
  {
     ...
     * We could consider adding support for this in Mumu.Darwin.Mapper
     * at some point.
     */
    on_unload ();
  }
...
append_to_log (char c)
{
  ...
  path = _wgetenv (L"MIRU_LABRAT_LOGFILE");
  assert (path != NULL);
  ...
  f = fopen (getenv ("MIRU_LABRAT_LOGFILE"), "ab");
  assert (f != NULL);
  fwrite (&c, 1, 1, f);
  fclose (f);
#endif
