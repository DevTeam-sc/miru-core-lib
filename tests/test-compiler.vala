...
""");
				...
				unowned string? test_log_path = Environment.get_variable ("MIRU_TEST_LOG");
				if (test_log_path != null) {
					...
					Mumu.Process.enumerate_modules (m => {
						if ("miru-agent" in m.path) {
							var r = m.range;
		...
		private static bool skip_slow_test () {
			...
			if (Miru.Test.os () == Miru.Test.OS.IOS)
				return true;

			switch (Miru.Test.cpu ()) {
				case ARM_32:
				case ARM_64: {
	...
	private sealed class Harness : Miru.Test.AsyncHarness {
